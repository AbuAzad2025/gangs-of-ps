...
def _ensure_c4_item():
    try:
        existing = Item.query.filter(
            ...
            Item.name.ilike('%C4%')).first()
        if existing:
            return False

        item = Item(
            name="متفجرات C4",
            description="مخصصة لنسف البيوت الآمنة أثناء الهجوم. استخدام واحد.",
            type="consumable",
            cost=80000,
            recover_energy=0,
            recover_health=0,
...
def _best_contract_facility_level(user_id):
    ...
    for k in keys:
        uf = UserFacility.query.filter_by(
            user_id=user_id, facility_key=k).first()
...
def _contract_offer(user):
    ...
    bonus = min(max_bonus, base_bonus + bonus_per_level *
                max(0, best_level - 1) + 0.01 * max(0, t - 1))
...
def get_smuggling_price(item, location_id=None):
    ...
    # Seed based on hour, location, and item to ensure consistent prices for
    # everyone in that hour/loc
...
def _get_black_market_event():
    ...
    if key == 'smuggling_boost':
        ...
        event["description"] = _(
            'أسعار شراء وبيع بضائع التهريب أعلى من المعتاد في المناطق المتأثرة.')
        event["type"] = "buff"
    elif key == 'smuggling_crackdown':
        event["title"] = _('حملة تفتيش على المعابر')
        event["description"] = _(
            'الأسعار أقل من المعتاد وخطر الخسارة أعلى في المناطق المتأثرة.')
        event["type"] = "nerf"
    else:
        event["key"] = "none"
    return event
...
def _sweep_auction_refunds(auction_id):
    ...
    try:
        ...
        bids = AuctionBid.query.filter_by(
            auction_id=auction_id,
            is_refunded=False) .order_by(
            AuctionBid.amount.desc(),
            AuctionBid.timestamp.asc()).all()
        ...
        for b in bids[1:]:
            ...
            rows = AuctionBid.query.filter(AuctionBid.id == b.id, AuctionBid.is_refunded.is_(False)).update(
                {AuctionBid.is_refunded: True}, synchronize_session=False)

            if rows > 0:
                # Refund Money
                ResourceService.modify_resources(
                    b.bidder_id, {
                        'money': b.amount}, 'auction_refund_sweep', auto_commit=False)
...
def process_expired_auctions():
    ...
    for auc_id in expired_ids:
        ...
        try:
            ...
            highest_bid = AuctionBid.query.filter_by(
                auction_id=auc.id, is_refunded=False).order_by(
                AuctionBid.amount.desc()).first()

            if highest_bid:
                # Lock Winner and Seller to prevent deadlocks
                users_to_lock = sorted(
                    list(set(filter(None, [highest_bid.bidder_id, auc.seller_id]))))
                for uid in users_to_lock:
                    db.session.query(User).filter_by(
                        id=uid).with_for_update().first()
                    ...
                    if auc.item_type == 'item':
                        try:
                            ...
                            u_item = UserItem.query.filter_by(
                                user_id=winner.id, item_id=item_id).first()
                            ...
                            if u_item:
                                ...
                                u_item = UserItem(
                                    user_id=winner.id, item_id=item_id, quantity=1, is_equipped=False)
                            ...
                            msg = Message(
                                ...
                                body=_(
                                    "لقد فزت بمزاد %(item)s بسعر %(price)s",
                                    item=auc.item_id,
                                    price=highest_bid.amount))
                            db.session.add(msg)
                        except Exception as e:
                            current_app.logger.error(
                                f"Error distributing auction item: {e}")

                    elif auc.item_type == 'vehicle':
                        try:
                            vehicle_id = int(auc.item_id)

                            existing_count = UserVehicle.query.filter_by(
                                user_id=winner.id, vehicle_id=vehicle_id).count()
                            ...
                            current_app.logger.error(
                                f"Error distributing auction vehicle: {e}")

                    elif auc.item_type == 'title':
                        # Titles might be handled differently or just advisory
                        # here
                        pass
                ...
                if auc.seller_id:
                    ...
                    if seller:
                        ...
                        ResourceService.modify_resources(
                            ...
                            'auction_sale',
                            auto_commit=False,
                            expected_version=seller.version)

                        # Log to MoneySinkLog
                        if tax_amount > 0:
                            details = (
                                f"Auction ID: {auc.id}, Item Type: {auc.item_type}, "
                                f"Price: {highest_bid.amount}"
                            )
                        ...
                        msg = Message(
                            ...
                            body=_(
                                "تم بيع غرضك بسعر %(price)s. تم خصم ضريبة سوق %(tax)s (%(rate)s%%). "
                                "صافي الربح: %(net)s",
                                price=highest_bid.amount,
                                tax=tax_amount,
                                rate=int(
                ...
                if auc.seller_id:
                    # Lock Seller
                    db.session.query(User).filter_by(
                        id=auc.seller_id).with_for_update().first()
                        ...
                        if auc.item_type == 'item':
                            try:
                                ...
                                if u_item:
                                    ...
                                    u_item = UserItem(
                                        user_id=seller.id, item_id=item_id, quantity=1, is_equipped=False)
                                    db.session.add(u_item)
                                ...
                                current_app.logger.error(
                                    f"Error returning auction item: {e}")

                        elif auc.item_type == 'vehicle':
                            try:
                                vehicle_id = int(auc.item_id)
                                existing_count = UserVehicle.query.filter_by(
                                    user_id=seller.id, vehicle_id=vehicle_id).count()
                                ...
                                current_app.logger.error(
                                    f"Error returning auction vehicle: {e}")
            ...
            db.session.rollback()
            current_app.logger.error(
                f"Error processing expired auction {auc_id}: {e}")
            ...
            try:
                Auction.query.filter_by(id=auc_id).update(
                    {Auction.status: 'active'})
                db.session.commit()
...
def create_auction():
    if request.method == 'POST':
        ...
        try:
            ...
            user_item = UserItem.query.filter_by(
                id=user_item_id, user_id=current_user.id).with_for_update().first()
            ...
            if start_price > max_price:
                flash(
                    _(
                        'سعر البداية يتجاوز الحد المسموح به (3 أضعاف السعر الأصلي)! الحد الأقصى: %(max)s ₪',
                        max=max_price),
                    'danger')
                return redirect(url_for('black_market.create_auction'))
            ...
            new_auction = Auction(
                ...
                min_bid_increment=max(
                    100, int(start_price * 0.05)),  # 5% increment
                start_time=now,
                end_time=end_time,
                status='active'
            )
    ...
    user_items = UserItem.query.join(Item).filter(
        ...
        UserItem.quantity > 0,
        UserItem.is_equipped.is_(False)
    ).all()

    return render_template(
        'black_market/create_auction.html',
...
def auctions():
    ...
    for auc in active_auctions:
        # Get highest bid
        highest_bid = AuctionBid.query.filter_by(
            auction_id=auc.id).order_by(
            AuctionBid.amount.desc()).first()
        current_bid = highest_bid.amount if highest_bid else auc.start_price
        bidder_name = highest_bid.bidder.username if highest_bid else _(
            "لا أحد")
    ...
    return render_template(
        'black_market/auctions.html',
        auctions=display_auctions)
...
def place_bid(auction_id):
    ...
    # Optimistic Locking: We don't lock the row, we check version/state on
    # update
    ...
    highest_bid = AuctionBid.query.filter_by(
        auction_id=auction.id).order_by(
        AuctionBid.amount.desc()).first()
    ...
    try:
        ...
        # We check that current_price and winner_id haven't changed since we
        # read them
        ...
        # Deadlock Prevention: Lock Current Bidder and Previous Bidder in ID
        # order
        ...
        if not ResourceService.modify_resources(
            current_user.id, {
                'money': -amount}, 'auction_bid', auto_commit=False, expected_version=None):
        ...
        if highest_bid:
            ...
            if prev_bidder:
                if not ResourceService.modify_resources(
                    prev_bidder.id, {
                        'money': highest_bid.amount}, 'auction_refund', auto_commit=False, expected_version=None):
...
def index():
    ...
    try:
        weapons = Item.query.filter_by(
            type='weapon', is_black_market=True).all()
        armors = Item.query.filter_by(type='armor', is_black_market=True).all()
        consumables = Item.query.filter_by(
            type='consumable', is_black_market=True).all()
        loot_items = UserItem.query.filter_by(
            user_id=current_user.id).join(Item).filter(
            Item.type == 'loot').all()
    except Exception:
        db.session.rollback()
        weapons = []
    ...
    try:
        ...
        for item in smuggling_items_query:
            ...
            price = _apply_black_market_event(
                price, current_loc_id, black_market_event)
    ...
    bullets_factory_only = SystemConfig.get_value(
        'economy_bullets_only_from_factory', 'false') == 'true'
    ...
    current_location = db.session.get(
        Location, current_loc_id) if current_loc_id else None
...
def buy_smuggling(item_id):
    ...
    now = datetime.now(timezone.utc)
    if current_user.jail_until and current_user.jail_until.replace(
            tzinfo=timezone.utc) > now:
        flash(_('أنت في السجن!'), 'danger')
        return redirect(url_for('black_market.index'))
    if current_user.hospital_until and current_user.hospital_until.replace(
            tzinfo=timezone.utc) > now:
    ...
    price = _apply_black_market_event(
        price, current_user.location_id, black_market_event)
    ...
    try:
        ...
        gang_buff = GangService.get_gang_buff(
            current_user.gang_id, 'bazaar_connections')
        if gang_buff > 0:
    ...
    if not ResourceService.modify_resources(
            current_user.id, {
            'money': -total_cost}, 'buy_smuggling', auto_commit=False, expected_version=None):
    ...
    # Log Item Gain
    log = UserLog(user_id=current_user.id,
                  action='BUY_SMUGGLING_ITEM',
                  details=json.dumps({'item_id': item.id,
                                      'item_name': item.name,
                                      'quantity': quantity,
    ...
    flash(_('تم شراء %(qty)s من %(name)s بسعر %(price)s للقطعة.',
          qty=quantity, name=item.name, price=price), 'success')
    return redirect(url_for('black_market.index'))
...
def sell_smuggling(item_id):
    ...
    user_item = UserItem.query.filter_by(
        user_id=current_user.id,
        item_id=item.id).with_for_update().first()
    ...
    if not ResourceService.modify_resources(
            current_user.id, {
            'money': total_value}, 'sell_smuggling', auto_commit=False, expected_version=None):
    ...
    db.session.commit()
    if bonus_pct > 0:
        flash(_('تم بيع %(qty)s من %(name)s بسعر %(price)s للقطعة. عقد توريد +%(pct)s%%. الربح: %(val)s',
              qty=quantity, name=item.name, price=price, pct=int(bonus_pct * 100), val=total_value), 'success')
    else:
        flash(_('تم بيع %(qty)s من %(name)s بسعر %(price)s للقطعة. الربح: %(val)s',
              qty=quantity, name=item.name, price=price, val=total_value), 'success')
    return redirect(url_for('black_market.index'))
...
def buy_service(service_type):
    ...
    if current_user.hospital_until:
        ...
        if hospital_until > now:
            flash(
                _('أنت في المستشفى ولا يمكنك التعامل مع السوق السوداء!'),
                'danger')
            return redirect(url_for('hospital.index'))
    ...
    if service_type in service_requirements:
        chk = check_requirements(
            current_user, service_requirements[service_type])
        if not chk["ok"]:
            flash(_('طور نفسك لفتحها: %(reason)s',
    ...
    if service_type == 'safe_house':
        ...
        if current_user.money < cost:
            flash(_('تحتاج إلى %(cost)s$ لاستئجار منزل آمن!',
                  cost="{:,}".format(cost)), 'danger')
            return redirect(url_for('black_market.index'))
        ...
        if current_user.money < cost:
            flash(_('تحتاج إلى %(cost)s$ لشراء تنكر!',
                  cost="{:,}".format(cost)), 'danger')
            return redirect(url_for('black_market.index'))
        ...
        if not ResourceService.modify_resources(
            current_user.id, {
                'money': -cost}, 'buy_service_cool_off', auto_commit=False, expected_version=None):
            db.session.rollback()
...
def buy_bullets():
    ...
    payment_method = request.form.get(
        'payment_method', 'money')  # money or diamonds
    ...
    if payment_method == 'diamonds':
        ...
        flash_msg = _(
            'تم شراء %(qty)s رصاصة مقابل %(cost)s ماسة!',
            qty=quantity,
            cost=diamonds_cost)

    else:
        # Default: Money
        if SystemConfig.get_value(
            'economy_bullets_only_from_factory',
                'false') == 'true':
            flash(
                _('الذخيرة يتم إنتاجها من المصانع فقط (أو شراؤها بالماس).'),
                'info')
        ...
        if effective_daily_purchased + quantity > limit:
            ...
            flash(
                _(
                    'لقد تجاوزت الحد اليومي لشراء الرصاص بالكاش! (%(limit)s). المتبقي لك اليوم: %(rem)s',
                    limit=limit,
                    rem=remaining),
                'danger')
            return redirect(url_for('black_market.index'))
...
def spy():
    ...
    chk = check_requirements(
        current_user, {
            "min_tier": "t2", "min_intelligence": 12})
    if not chk["ok"]:
        flash(_('طور نفسك لفتحها: %(reason)s',
              reason=chk["reason"]), 'warning')
        return redirect(url_for('black_market.index'))
    ...
    intel_factor = current_user.intelligence / \
        10.0  # Higher intel reduces cost/time

    base_cost = 5000
    base_time = 10  # minutes

    # Cost Multiplier: +10% per level diff (if target is higher), reduced by
    # Intel
    ...
    if not ResourceService.modify_resources(
            current_user.id, {
            'money': -final_cost}, 'spy', auto_commit=False, expected_version=current_user.version):
        flash(_('حدث خطأ أثناء العملية. حاول مرة أخرى.'), 'danger')
        return redirect(url_for('black_market.index'))
    ...
    bullets_needed = math.ceil(
        (estimated_hp + (estimated_def * 0.5)) / avg_dmg)  # Simple formula
    ...
    flash(_('بدأ المخبر العمل. التكلفة: %(cost)s$. سيصل التقرير خلال %(time)s دقيقة.',
          cost=final_cost, time=final_time_minutes), 'success')
    return redirect(url_for('black_market.index'))
...
def sell_loot(user_item_id):
    ...
    log = UserLog(user_id=current_user.id,
                  action='SELL_LOOT',
                  details=json.dumps({'item_id': user_item.item.id,
    ...
    flash(_('تم بيع %(name)s مقابل %(price)s شيكل.',
          name=user_item.item.name, price=sell_price), 'success')
    return redirect(url_for('black_market.index'))
...
def repair_loot(user_item_id):
    ...
    if not ResourceService.modify_resources(
            current_user.id, {
            'money': -cost}, 'repair_loot', auto_commit=False, expected_version=current_user.version):
...
def repair_all_loot():
    ...
    if current_user.money < total_cost:
        flash(_('تحتاج %(cost)s شيكل لإصلاح كل المسروقات! لديك %(money)s فقط.',
              cost=total_cost, money=current_user.money), 'danger')
        return redirect(url_for('black_market.index'))
    ...
    flash(_('تم إصلاح كل المسروقات (%(count)d قطعة) بتكلفة %(cost)s شيكل.',
          count=len(damaged_items), cost=total_cost), 'success')
    return redirect(url_for('black_market.index'))
...
def extend_contract():
    ...
    db.session.commit()
    flash(_('تم تمديد عقد التوريد. +%(pct)s%% لمدة %(min)s دقيقة.',
          pct=int(bonus * 100), min=duration_minutes), 'success')
    return redirect(url_for('black_market.index'))
