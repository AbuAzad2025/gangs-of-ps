...
def _calc_upgrade_caps(vehicle):
    ...
    engine_max = 4 + int(round((10 * price_score) + (4 * speed_score) - (2 * risk_score)))
    tires_max = 4 + int(round((9 * price_score) + (5 * speed_score) - (2 * risk_score)))
    armor_max = 3 + int(round((9 * price_score) + (5 * defense_score) - (3 * risk_score)))
    ...
    return {'engine': int(engine_max), 'tires': int(tires_max), 'armor': int(armor_max)}
...
def index():
    ...
    if ai_unlimited:
        ...
        ai_used = {
            'easy': _ai_race_used_today(current_user.id, 'easy') if ai_limits.get('easy', 0) > 0 else 0,
            'normal': _ai_race_used_today(current_user.id, 'normal') if ai_limits.get('normal', 0) > 0 else 0,
            'hard': _ai_race_used_today(current_user.id, 'hard') if ai_limits.get('hard', 0) > 0 else 0,
        }
    if my_car:
        ...
        if skill < max_skill:
            ...
            for d in defs:
                ...
                if gain <= 0:
                    continue
                cost = _round_money(base_per_skill * skill * float(d['factor']) * (gain / float(d['gain'])), 100)
                training_options.append({'key': d['key'], 'label': d['label'], 'gain': gain, 'cost': cost})
...
def create():
    ...
    existing_race = Race.query.filter_by(creator_id=current_user.id, status='waiting').first()
    if existing_race:
        flash(_('عندك سباق بانتظار المتحدين بالفعل. احذفه أو استنى حد ينضم.'), 'warning')
...
def cancel(race_id):
    ...
    if not ResourceService.modify_resources(current_user.id, {'money': race.bet_amount}, 'cancel_race_refund', auto_commit=False, expected_version=None):
...
def start(race_id):
    ...
    for p in participants:
        ...
        pre_condition_score = base_speed + engine_bonus + tires_bonus + driver_bonus + luck
    ...
    for i, p in enumerate(results):
        ...
        try:
            detail_log = (
                UserLog.query.filter_by(user_id=p.user_id, action='RACE_PVP_RESULT')
                .order_by(UserLog.timestamp.desc())
                .first()
            )
            if detail_log:
...
def train_driver():
    ...
    cost = _round_money(1000 * skill * factor * (gain / float(desired_gain)), 100)
    ...
    current_user.driving_skill = min(max_skill, current_user.driving_skill + gain)
    db.session.commit()
...
def ai_race():
    ...
    if not _is_unlimited_user(current_user):
        ...
        if daily_limit > 0:
            ...
            if used_today >= daily_limit:
                flash(_('وصلت للحد اليومي لسباقات الكمبيوتر (%(limit)s) لهذه الصعوبة. ارجع بكرة!', limit=daily_limit), 'warning')
                return redirect(url_for('racing.index'))
    ...
    payout_mult = {'easy': 1.5, 'normal': 2.0, 'hard': 2.6}.get(difficulty, 2.0)

    if not ResourceService.modify_resources(current_user.id, {'money': -bet}, f'ai_race_bet_{difficulty}', auto_commit=False, expected_version=None):
        flash(_('معكش مصاري كفاية!'), 'danger')
        return redirect(url_for('racing.index'))

    def calc_score(base_speed, engine_level, tires_level, driver_skill, condition, luck_bonus=0):
        luck = random.randint(0, 20) + int(luck_bonus)
        total = base_speed + (engine_level * 5) + (tires_level * 3) + (driver_skill * 2) + luck
    ...
    player_score = calc_score(base_speed, my_engine, my_tires, my_skill, my_car.condition, luck_bonus=luck_bonus)

    opponents = []
    opp_count = 3
    diff_factor = {'easy': 0.92, 'normal': 1.02, 'hard': 1.12}.get(difficulty, 1.02)
    caps = _calc_upgrade_caps(my_car.vehicle)
    for i in range(opp_count):
        o_engine = min(caps['engine'], max(0, int(round(my_engine * diff_factor + random.randint(-2, 2)))))
        o_tires = min(caps['tires'], max(0, int(round(my_tires * diff_factor + random.randint(-2, 2)))))
        o_skill = max(1, int(round(my_skill * diff_factor + random.randint(-3, 4))))
        o_cond = max(75, min(100, int(97 + random.randint(-12, 4))))
        o_speed = max(5, int(round(base_speed * diff_factor + random.randint(-7, 10))))
        score = calc_score(o_speed, o_engine, o_tires, o_skill, o_cond, luck_bonus=0)
        opponents.append(
            {
                'name': f'AI-{i+1}',
                'score': score,
    ...
    if win:
        ...
        ResourceService.modify_resources(current_user.id, {'money': winnings}, f'ai_race_win_{difficulty}', auto_commit=False, expected_version=None)
        flash(_('فزت على الكمبيوتر! ربحت %(amt)s شيكل.', amt=winnings), 'success')
    else:
        flash(_('خسرت قدام الكمبيوتر! الفائز: %(name)s', name=best_opp['name']), 'danger')
