...
from models import User, Gang, CombatLog, UserLog
...
def trend():
    ...
    recent_battles = CombatLog.query.order_by(CombatLog.timestamp.desc()).limit(10).all()
    
    # 2. Top Gangs
    top_gangs = Gang.query.order_by(Gang.level.desc(), Gang.exp.desc()).limit(5).all()
    ...
    seo_manager.set(
        ...
        image=url_for('static', filename='images/trend_share.jpg', _external=True)
    )
    seo_manager.add_breadcrumb(_("الترند"), url_for('main.trend', _external=True))
