    ...
    try:
        if room.game_type == 'chess':
            ...
            if state.get('team_mode') == 'partnership':
                ...
                winning_seats = [0, 2] if score_a > score_b else (
                    [1, 3] if score_b > score_a else [0, 1, 2, 3])
            else:
                max_score = max(scores) if scores else 0
                winning_seats = [
                    i for i, s in enumerate(scores) if s == max_score]
        ...
        if room.game_type == 'chess' and state.get(
                'result') == '1/2-1/2' and state.get('is_solo'):
        ...
        # We hold the lock here, and ResourceService will be able to operate
        # safely
        db.session.query(User).filter(
            User.id.in_(sorted_ids)).order_by(
        ...
        if azad_id and azad_id in users_to_lock and house_cut > 0:
            changes = {
                'money': house_cut} if room.currency_type == 'money' else {
                'diamonds': house_cut}
        ...
        if success and azad_id and azad_id in users_to_lock and remainder > 0:
            changes = {
                'money': remainder} if room.currency_type == 'money' else {
                'diamonds': remainder}
            if not ResourceService.modify_resources(
                azad_id,
                changes,
        ...
        if success:
            for uid in winner_user_ids:
                changes = {
                    'money': share_per_winner} if room.currency_type == 'money' else {
                    'diamonds': share_per_winner}
                if not ResourceService.modify_resources(
                        uid, changes, 'game_win_prize', auto_commit=False, expected_version=None):
                    success = False
                    break
        ...
        if success:
            ...
            current_app.logger.error(
                "Failed to distribute prizes via ResourceService")
