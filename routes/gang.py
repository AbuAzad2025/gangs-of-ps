...
from models.social import Gang, GangInvite, GangLog, GangWar, GangAlliance, GangItem, GangChat
...
@login_required
def invites():
    invites = GangInvite.query.filter_by(
        user_id=current_user.id, status='pending').all()
    return render_template(
        'gang/invites.html',
        title=_('دعوات الانضمام'),
        invites=invites)
...
def accept_invite(invite_id):
    ...
    # Lock invite
    invite = db.session.query(GangInvite).filter_by(
        id=invite_id).with_for_update().first()
    if not invite:
        abort(404)
    ...
    gang = db.session.query(Gang).filter_by(
        id=invite.gang_id).with_for_update().first()
    if not gang:
    ...
    log = GangLog(
        ...
        action=_('انضم للعصابة عبر دعوة'))
    db.session.add(log)
    db.session.commit()
...
def leave():
    ...
    if current_user.id == gang.leader_id:
        flash(
            _('لا يمكنك مغادرة العصابة وأنت الزعيم! يجب عليك نقل الزعامة أو حل العصابة.'),
            'danger')
        return redirect(url_for('gang.dashboard'))
...
def create():
    ...
    if request.method == 'POST':
        ...
        if current_user.money < cost:
            flash(_('لا تملك كاش كافي لإنشاء عصابة! المطلوب: %(cost)s',
                  cost=cost), 'danger')
            return redirect(url_for('gang.create'))
        ...
        if cost > 0:
            ...
            if not ResourceService.modify_resources(
                current_user.id, {
                    'money': -cost}, 'gang_creation', auto_commit=False, expected_version=None):
                db.session.rollback()
        ...
        new_gang.recruitment_status = request.form.get(
...
def request_join(gang_id):
    ...
    if current_user.level < gang.min_level_req:
        flash(_('مستواك أقل من الحد الأدنى المطلوب للانضمام (مستوى %(level)s).',
              level=gang.min_level_req), 'danger')
        return redirect(url_for('gang.view', gang_id=gang.id))

    if gang.allowed_countries:
        allowed = [c.strip().upper()
                   for c in gang.allowed_countries.split(',')]
    ...
    log = GangLog(
        gang_id=gang.id,
        user_id=current_user.id,
        action="Joined the gang directly")
    db.session.add(log)
    db.session.commit()
...
def kick_member(user_id):
    ...
    member_to_kick = db.session.query(User).filter_by(
    ...
    if gang.money < penalty_cost:
        flash(_('خزينة العصابة لا تكفي لدفع مستحقات الطرد! (%(cost)s$)',
              cost=penalty_cost), 'warning')
        return redirect(url_for('gang.dashboard'))

    # Atomic update for gang money
    g_rows = Gang.query.filter(Gang.id == gang.id, Gang.money >= penalty_cost).update(
        {Gang.money: Gang.money - penalty_cost}, synchronize_session=False)
    ...
    ResourceService.modify_resources(member_to_kick.id,
                                     {'money': refund_amount},
                                     'gang_kick_severance',
                                     auto_commit=False,
...
def dashboard():
    ...
    logs = GangLog.query.filter_by(
        gang_id=gang.id).order_by(
        GangLog.timestamp.desc()).limit(20).all()
...
def buy_upgrade(upgrade_id):
    level = request.args.get(
        'level', type=int) or request.form.get(
        'level', type=int)
    ...
    upgrade_def = next(
        (u for u in upgrades_data if u.get('id') == upgrade_id), None)
    if not upgrade_def:
        flash(_('التطوير غير موجود.'), 'danger')
        return redirect(url_for('gang.dashboard'))

    level_def = next(
        (level_row for level_row in (upgrade_def.get('levels') or []) if int(
            level_row.get('level') or 0) == int(level)),
        None)
    if not level_def:
    ...
    if cost > 0:
        rows = Gang.query.filter(Gang.id == gang.id, Gang.money >= cost).update(
            {Gang.money: Gang.money - cost}, synchronize_session=False)
        if rows == 0:
            flash(_('لا يوجد مال كافٍ في خزينة العصابة!'), 'danger')
            return redirect(url_for('gang.dashboard'))
    ...
    db.session.add(
        GangLog(
            gang_id=gang.id,
            user_id=current_user.id,
            action=f"Upgraded {upgrade_id} to {level}"))
    db.session.commit()
...
def broadcast_invite_all():
    ...
    is_admin = current_user.role.value >= UserRole.ADMIN.value
    if current_user.id != gang.leader_id and current_user.id != getattr(
            gang, "underboss_id", None) and not is_admin:
        flash(_('فقط القيادة يمكنها إرسال دعوات عامة!'), 'danger')
        return redirect(url_for('gang.dashboard'))

    cost = 50000
    if gang.money < cost:
        flash(_('لا يوجد رصيد كافي في خزينة العصابة! التكلفة: %(cost)s$',
              cost=cost), 'danger')
        return redirect(url_for('gang.dashboard'))
...
def view(gang_id):
    ...
    meta_description = _(
        "تفاصيل عصابة %(name)s. المستوى: %(level)s, الأعضاء: %(count)s. انضم الآن وسيطر!",
        name=gang.name,
        level=gang.level,
        count=members_count)
...
def declare_war(target_gang_id):
    ...
    existing_war = GangWar.query.filter(
        or_(
            (GangWar.gang1_id == my_gang.id) & (
                GangWar.gang2_id == target_gang.id),
            (GangWar.gang1_id == target_gang.id) & (
                GangWar.gang2_id == my_gang.id)),
    ...
    alliance = GangAlliance.query.filter(
        ((GangAlliance.gang1_id == my_gang.id) & (
            GangAlliance.gang2_id == target_gang.id)) | (
            (GangAlliance.gang1_id == target_gang.id) & (
                GangAlliance.gang2_id == my_gang.id)),
    ...
    if my_gang.level > (target_gang.level * 2 + 5):
        flash(
            _('عصابتك قوية جداً مقارنة بالخصم! لا يمكن إعلان الحرب (نظام اللعب النظيف).'),
            'warning')
        return redirect(url_for('gang.view', gang_id=target_gang_id))
    ...
    if recent_war:
        flash(
            _('لا يمكن إعلان الحرب الآن! توجد هدنة مؤقتة (24 ساعة) بعد الحرب الأخيرة.'),
            'warning')
        return redirect(url_for('gang.view', gang_id=target_gang_id))
    ...
    log = GangLog(
        ...
        user_id=current_user.id,
        action=_(
            'أعلن الحرب على %(gang)s',
            gang=target_gang.name))
    db.session.add(log)
    db.session.commit()
...
def surrender_war(war_id):
    ...
    war = db.session.query(GangWar).filter_by(
        id=war_id).with_for_update().first()
    ...
    if reparation_amount > 0:
        rows = Gang.query.filter(Gang.id == gang.id, Gang.money >= reparation_amount).update(
            {Gang.money: Gang.money - reparation_amount}, synchronize_session=False, )
        if rows == 0:
            current_money = int(getattr(gang, "money", 0) or 0)
            if current_money > 0:
                Gang.query.filter(Gang.id == gang.id).update(
                    {Gang.money: 0},
                    synchronize_session=False,
                )
                reparation_amount = current_money
            else:
                reparation_amount = 0

    if reparation_amount > 0:
        Gang.query.filter(Gang.id == opponent_gang.id).update(
            {Gang.money: Gang.money + reparation_amount},
            synchronize_session=False,
        )
    ...
    log_loser = GangLog(
        ...
        user_id=current_user.id,
        action=_(
            'استسلم في الحرب ضد %(opp)s ودفع تعويضات %(amount)s$',
            opp=opponent_gang.name,
            amount=reparation_amount))
    log_winner = GangLog(
        gang_id=opponent_id,
        action=_(
            'انتصر في الحرب بعد استسلام %(loser)s وحصل على تعويضات %(amount)s$',
            loser=gang.name,
            amount=reparation_amount))
...
def donate():
    ...
    if not ResourceService.modify_resources(
            current_user.id, {
            'money': -amount}, 'gang_donate', auto_commit=False, expected_version=current_user.version):
        flash(_('لا تملك هذا المبلغ!'), 'danger')
        return redirect(url_for('gang.dashboard'))
...
def withdraw():
    ...
    if not ResourceService.modify_resources(
            current_user.id, {
            'money': amount}, 'gang_withdraw', auto_commit=False, expected_version=None):
        db.session.rollback()
...
def do_organized_crime(crime_id):
    ...
    if members_count < crime.min_members and not is_admin:
        flash(_('عدد أعضاء العصابة غير كافي لهذه العملية. المطلوب: %(min)s',
              min=crime.min_members), 'danger')
        return redirect(url_for('gang.organized_crimes'))
    ...
    if gang.last_organized_crime_at and not is_admin:
        ...
        if elapsed < timedelta(hours=crime.cooldown_hours):
            wait_hours = crime.cooldown_hours - \
                (elapsed.total_seconds() / 3600)
            flash(_('العصابة في فترة راحة. يجب الانتظار %(hours).1f ساعة.',
                  hours=wait_hours), 'warning')
            return redirect(url_for('gang.organized_crimes'))
    ...
    codes = set([c.strip().upper() for c in invite_countries_raw.split(
        ',') if c.strip()]) if invite_countries_raw else set()

    # Build recipients list
    if invite_scope == 'gang':
        recipients = User.query.filter_by(
            gang_id=gang.id).filter(
    ...
    if codes:
        recipients = [
            u for u in recipients if (
                u.country or '').upper() in codes]
    ...
    # Log
    log = GangLog(gang_id=gang.id, user_id=current_user.id, action=_(
        'أنشأ مجموعة لجريمة منظمة: %(crime)s وأرسل دعوات عامة', crime=crime.name))
    db.session.add(log)
    db.session.commit()
    ...
    if upgrade_type == 'capacity':
        ...
        base = int(
            SystemConfig.get_value(
                'gang_upgrade_capacity_base_cost',
                1000))
        ...
        if gang.money < cost:
            flash(_('لا يوجد مال كافٍ في الخزينة! التكلفة: %(cost)s$',
                  cost=cost), 'danger')
            return redirect(url_for('gang.dashboard'))
        ...
        gang.max_members += int(
            SystemConfig.get_value(
                'gang_upgrade_capacity_amount', 5))
        ...
        flash(_('تم زيادة سعة العصابة إلى %(num)s عضو!',
              num=gang.max_members), 'success')

    elif upgrade_type == 'level':
        base_money = int(
            SystemConfig.get_value(
                'gang_upgrade_level_base_money',
                10000))
        base_exp = int(
            SystemConfig.get_value(
                'gang_upgrade_level_base_exp', 500))
        ...
        if rows == 0:
            if gang.money < money_cost:
                flash(_('لا يوجد مال كافٍ في الخزينة! التكلفة: %(cost)s$',
                      cost=money_cost), 'danger')
            elif gang.exp < exp_cost:
                flash(_('لا يوجد خبرة كافية للعصابة! المطلوب: %(cost)s XP',
                      cost=exp_cost), 'danger')
            else:
        ...
        log = GangLog(
            ...
            user_id=current_user.id,
            action=_(
                'طوّر مستوى العصابة إلى %(num)s',
                num=gang.level))
        db.session.add(log)
        db.session.commit()

        flash(_('تم تطوير مستوى العصابة إلى المستوى %(num)s!',
              num=gang.level), 'success')
...
def request_alliance(target_gang_id):
    ...
    existing = GangAlliance.query.filter(
        ((GangAlliance.gang1_id == my_gang.id) & (
            GangAlliance.gang2_id == target_gang.id)) | (
            (GangAlliance.gang1_id == target_gang.id) & (
                GangAlliance.gang2_id == my_gang.id))).first()
    ...
    my_active_count = GangAlliance.query.filter(
        ((GangAlliance.gang1_id == my_gang.id) | (
            GangAlliance.gang2_id == my_gang.id)),
        GangAlliance.status == 'active').count()

    if my_active_count >= MAX_ALLIANCES:
        flash(_('لقد وصلت عصابتك للحد الأقصى من التحالفات (%(max)s).',
              max=MAX_ALLIANCES), 'warning')
        return redirect(url_for('gang.index'))
    ...
    if target_active_count >= MAX_ALLIANCES:
        flash(_('العصابة المستهدفة وصلت للحد الأقصى من التحالفات.',
              max=MAX_ALLIANCES), 'warning')
        return redirect(url_for('gang.index'))

    alliance = GangAlliance(
        gang1_id=my_gang.id,
        gang2_id=target_gang.id,
        status='pending')
...
def alliances():
    ...
    incoming = GangAlliance.query.filter_by(
        gang2_id=gang_id, status='pending').all()

    # Active Alliances
    active = GangAlliance.query.filter(
        ((GangAlliance.gang1_id == gang_id) | (
            GangAlliance.gang2_id == gang_id)),
        GangAlliance.status == 'active').all()
...
def disband():
    ...
    # Delete gang; cascade should handle related items (invites/logs) if configured
...
def transfer_leadership():
    ...
    log = GangLog(
        ...
        user_id=current_user.id,
        action=_(
            'نقل الزعامة إلى %(user)s',
            user=new_leader.username))
    db.session.add(log)

    db.session.commit()

    flash(_('تم نقل زعامة العصابة إلى %(user)s بنجاح.',
          user=new_leader.username), 'success')
    return redirect(url_for('gang.dashboard'))
...
def donate_item():
    ...
    # Add to gang
    gang_item = GangItem.query.filter_by(
        gang_id=current_user.gang_id,
    ...
    log = GangLog(
        ...
        user_id=current_user.id,
        action=_(
            'تبرع بـ %(qty)s x %(item)s للعصابة',
            qty=quantity,
            item=item_name))
    db.session.add(log)
...
def distribute_item():
    ...
    GangItem.query.filter(
        GangItem.gang_id == gang.id,
        GangItem.item_id == item_id,
        GangItem.quantity == 0).delete(
        synchronize_session=False)

    # Add to user
    user_item = UserItem.query.filter_by(
        user_id=target_user.id, item_id=item_id).first()
    if user_item:
        user_item.quantity += quantity
    else:
        user_item = UserItem(
    ...
    log = GangLog(
        ...
        user_id=current_user.id,
        action=_(
            'أعطى %(qty)s x %(item)s للعضو %(user)s',
            qty=quantity,
            item=item_name,
...
def distribute_money():
    ...
    if not ResourceService.modify_resources(
        ...
        'gang_distribution',
        auto_commit=False,
            expected_version=target_user.version):
    ...
    log = GangLog(
        ...
        user_id=current_user.id,
        action=_(
            'حول %(amount)s$ للعضو %(user)s',
            amount=amount,
            user=target_user.username))
    db.session.add(log)

    db.session.commit()
    flash(_('تم تحويل %(amount)s$ للعضو %(user)s بنجاح!',
          amount=amount, user=target_user.username), 'success')
    return redirect(url_for('gang.dashboard'))
...
def chat_messages():
    ...
    query = GangChat.query.filter(GangChat.gang_id == current_user.gang_id)

    if since_id > 0:
        query = query.filter(GangChat.id > since_id)

    # Get last 50 messages if no since_id, otherwise get all new messages
    if since_id == 0:
        messages = query.order_by(
            GangChat.created_at.desc()).limit(limit).all()
        messages.reverse()  # Show oldest to newest
    else:
        messages = query.order_by(GangChat.created_at.asc()).limit(limit).all()
...
def chat_delete(msg_id):
    ...
    msg = db.session.get(GangChat, msg_id)
    if not msg or msg.gang_id != current_user.gang_id:
    ...
    is_admin = current_user.role.value >= UserRole.MODERATOR.value
