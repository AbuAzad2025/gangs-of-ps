...
def save_image(form_image, folder):
    ...
    # Handle case where form_image is a string (e.g. existing filename from
    # obj)
    ...
    # Add random string for extra safety
...
def update_daily_task_progress(user, target_type):
    ...
    try:
        ...
        if current_app.debug:
            current_app.logger.debug(
                f"Updating daily task for user {user.id}, type: {target_type} -> {target_types}")
        ...
        tasks = UserDailyTask.query.join(DailyTask).filter(
            ...
            UserDailyTask.is_completed.is_(False),
        ...
        if current_app.debug and updated_count > 0:
            current_app.logger.debug(
                f"Updated {updated_count} tasks for user {user.id}")
...
def sync_daily_tasks(user):
    ...
    try:
        ...
        user_tasks = UserDailyTask.query.filter_by(
            user_id=user.id, date=today).all()
        if user_tasks:
            return user_tasks
        ...
        if not all_tasks:
            current_app.logger.warning(
                "No daily tasks found in DB. Attempting initialization.")
            try:
                ...
                current_app.logger.error(
                    f"Failed to initialize daily tasks: {e}")
                db.session.rollback()

        if not all_tasks:
            current_app.logger.error(
                "Still no daily tasks available after initialization attempt.")
            return []
        ...
        db.session.commit()
        current_app.logger.info(
            f"Assigned {len(new_tasks)} daily tasks to user {user.id}")
