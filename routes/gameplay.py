...
def organized_crimes():
    if not current_user.is_authenticated:
        ...
        crimes = OrganizedCrime.query.filter_by(
            is_active=True).order_by(
            OrganizedCrime.min_level).all()
        meta_description = _(
        ...
        crime_meta = {}
        for c in crimes:
            crime_access[c.id] = {"unlocked": False,
                                  "reason": _("يجب تسجيل الدخول للعب"),
                                  "hint_text": _("سجل دخولك الآن"),
                                  "hint_url": url_for('main.login')}
            # Basic meta
            reqs = {}
            try:
                import json
        ...
        return render_template(
            'organized_crimes.html',
            crimes=crimes,
            allow_non_gang=True,
            can_create=False,
            min_creator_rank_level=0,
            active_lobbies=[],
            crime_meta=crime_meta,
            crime_access=crime_access,
            lobby_counts={},
    ...
    if existing_participation:
        ...
        if lobby and not _expire_lobby_if_needed(lobby) and lobby.status in [
    ...
    enabled = SystemConfig.get_value(
        'organized_crimes_enabled', 'true') == 'true'
    allow_non_gang = SystemConfig.get_value(
        'organized_crimes_allow_non_gang', 'true') == 'true'
    ...
    user_cooldowns = UserOrganizedCrimeCooldown.query.filter_by(
    ...
    for c in crimes:
        ...
        if not chk["ok"]:
            ...
            hint_url = url_for('main.daily_tasks') if chk.get(
                "hint_key") == "daily_tasks" else None
        ...
        if is_ok:
            ...
            if roles:
                for role in roles:
                    role_name = role.get("name") if isinstance(
                        role, dict) else None
                    role_key = role.get("key") if isinstance(
                        role, dict) else None
                    r = {}
                    ...
                    if isinstance(r, dict):
                        req_map = {}
                        if "strength" in r:
                            req_map["min_strength"] = r.get("strength")
                        if "agility" in r:
                            req_map["min_agility"] = r.get("agility")
                        if "intelligence" in r:
                            req_map["min_intelligence"] = r.get("intelligence")
        ...
        crime_access[c.id] = {"unlocked": bool(
            is_ok), "reason": reason, "hint_text": hint_text, "hint_url": hint_url}

    effective_level = current_user.level
    try:
        effective_level = current_user.level + \
            (current_user.rank_points_value // 50)
    except BaseException:
        pass
    ...
    return render_template(
        'organized_crimes.html',
        crimes=crimes,
        allow_non_gang=allow_non_gang,
        can_create=can_create,
        min_creator_rank_level=min_creator_rank_level,
        active_lobbies=active_lobbies,
        crime_meta=crime_meta,
        crime_access=crime_access,
        lobby_counts=lobby_counts)
...
def _expire_lobby_if_needed(lobby):
    ...
    try:
        ...
        now = datetime.now(timezone.utc)
        if now - \
                created > timedelta(hours=24) and lobby.status == 'recruiting':
...
def collect_task_reward(task_id):
    ...
    user_task = UserDailyTask.query.filter_by(
        id=task_id).with_for_update().first()
    ...
    flash(_('مبروك! حصلت على المكافأة: %(money)s شيكل و %(exp)s خبرة.',
            money=user_task.task.reward_money,
            exp=user_task.task.reward_exp),
          'success')
    return redirect(url_for('main.daily_tasks'))
...
def daily_reward():
    ...
    # Re-fetch user to ensure we are working on the locked instance in this
    # session
...
def hara():
    ...
    if last_claim_at:
        ...
        if now - last_claim_at < timedelta(hours=24):
            ...
            minutes, remaining_seconds = divmod(remainder, 60)
            remaining_text = _(
                '%(hours)s ساعة و %(minutes)s دقيقة',
                hours=hours,
                minutes=minutes)
...
def empire():
    ...
    try:
        active_factory_job = FactoryJob.query.filter_by(
            user_id=current_user.id, status='running').order_by(
            FactoryJob.ends_at.desc()).first()
        factory_jobs_count = FactoryJob.query.filter_by(
    ...
    try:
        contracts = FarmSupplyContract.query.filter_by(
            user_id=current_user.id).order_by(
            FarmSupplyContract.ends_at.desc()).all()
    except Exception:
        try:
            db.session.rollback()
        except Exception:
    ...
    try:
        smuggling_items = UserItem.query.join(Item).filter(
            UserItem.user_id == current_user.id, Item.type == 'smuggling').all()
    except Exception:
        try:
            db.session.rollback()
        except Exception:
...
def crimes():
    ...
    all_crimes = Crime.query.filter_by(
        is_active=True).order_by(
        Crime.min_level).all()
    ...
    # Get user cooldowns
    cooldowns = UserCrimeCooldown.query.filter_by(
    ...
    if current_user.crime_cooldown_until:
        ...
        # Ensure 'until' is timezone-aware (UTC) for comparison and timestamp
        # calculation
...
def do_crime(crime_id):
    ...
    if current_user.crime_cooldown_until:
        ...
        # This was causing timezone issues (6618 seconds block) and users want
    ...
    if effective_strength < crime.min_strength:
        flash(_('عضلاتك ضعيفة! تحتاج قوة %(str)s (مع العتاد) لتنفيذ هذه الجريمة',
              str=crime.min_strength), 'danger')
        return redirect(url_for('main.crimes'))
    ...
    user_cooldown = UserCrimeCooldown.query.filter_by(
        user_id=current_user.id, crime_id=crime.id).first()

    if daily_limit > 0:
        if user_cooldown and user_cooldown.last_reset_date == now.date():
            if user_cooldown.daily_count >= daily_limit:
                flash(_('لقد وصلت للحد اليومي لهذه الجريمة! (%(limit)s مرة يومياً)',
                      limit=daily_limit), 'danger')
                return redirect(url_for('main.crimes'))
    ...
    if user_cooldown:
        ...
        if cooldown_until > now:
            ...
            flash(_('عليك الانتظار %(time)s ثانية قبل القيام بهذه الجريمة مرة أخرى!', time=int(
                remaining)), 'danger')
            return redirect(url_for('main.crimes'))
    ...
    try:
        base_global_cd_seconds = int(
            SystemConfig.get_value(
                "crime_global_cooldown_seconds",
                str(base_global_cd_seconds)) or base_global_cd_seconds)
    except Exception:
    ...
    try:
        max_global_cd_seconds = int(
            SystemConfig.get_value(
                "crime_global_cooldown_max_seconds",
                str(max_global_cd_seconds)) or max_global_cd_seconds)
    except Exception:
    ...
    # Smart Ceiling: As heat approaches 100, success drops drastically but
    # never hits 0 purely by heat (min 5%)
    ...
    if heat > 40 and random.random() < 0.05:
        jail_time = 60
        # Atomic update handled by direct assignment here since we commit
        # immediately
        current_user.jail_until = datetime.now(
            timezone.utc) + timedelta(minutes=jail_time)
        # We might want to clear the cooldown to let them suffer in jail but
        # not wait after? No, keep CD.
        db.session.commit()
    ...
    if roll <= final_chance:
        ...
        if hasattr(crime, 'reward_type') and crime.reward_type == 'vehicle':
            ...
            available_vehicles = Vehicle.query.filter(
                Vehicle.price <= max_price, Vehicle.is_active).all()
            ...
            if item_reward:
                if not current_app.config.get('TESTING', False):
                    if random.random() < 0.15:
                        ...
                        item_condition = random.randint(
                            55, 90) if item_reward.type == 'loot' else random.randint(
                            70, 95)
                ...
                existing_item = UserItem.query.filter_by(
                    user_id=current_user.id, item_id=item_reward.id).first()
                if existing_item:
                    existing_item.quantity += 1
                    if existing_item.condition is not None:
                        existing_item.condition = min(
                            existing_item.condition, item_condition)
                else:
        ...
        try:
            try:
                heat_base = int(
                    SystemConfig.get_value(
                        "crime_heat_gain_base",
                        "2") or 2)
            except BaseException:
                heat_base = 2
            heat_gain = heat_base + \
                int(crime.min_level / 10) + int(crime.energy_cost / 20)
        ...
        if story_vehicle_name:
            parts.append({'msgid': 'سرقت %(name)s.',
                          'params': {'name': story_vehicle_name}})
            parts.append({'msgid': 'بس حالتها %(cond)s%% وبدها تصليح بالمرآب.', 'params': {
                         'cond': story_vehicle_condition}})
        ...
        if story_item_name:
            parts.append({'msgid': 'لقيت %(item)s.',
                         'params': {'item': story_item_name}})
            if story_item_condition is not None and story_item_condition < 100:
                parts.append({'msgid': 'كانت حالتها %(cond)s%%، بدها تصليح قبل البيع.', 'params': {
                             'cond': story_item_condition}})
        ...
        parts.append({'msgid': 'وكسبت %(money)s شيكل و %(exp)s خبرة.',
                     'params': {'money': money, 'exp': xp_reward}})
        ...
        if gang_bonus_percent > 0:
            parts.append({'msgid': '(بونص عصابة +%(bonus)s%%)',
                         'params': {'bonus': gang_bonus_percent}})
        if leveled_up:
            parts.append({'msgid': 'مبروك! وصلت للمستوى %(level)s.',
                         'params': {'level': current_user.level}})
...
def equip_item(item_id):
    ...
    user_item = db.session.query(UserItem).filter_by(
        id=item_id).with_for_update().first()
    if not user_item:
        abort(404)
    ...
    # We also need to lock the currently equipped item to prevent race
    # conditions there
...
def hack_bank():
    ...
    if current_user.intelligence < 50:
        db.session.rollback()
        flash(
            _('ذكاؤك لا يكفي لاختراق البنك المركزي! تحتاج 50 ذكاء على الأقل.'),
            'danger')
        return redirect(url_for('main.intel_center'))

    # Chance
    # Base 30% + 1% per intel over 50
    chance = 30 + (current_user.intelligence - 50)
...
def create_lobby(crime_id):
    ...
    # We use a dummy update or select for update on User to serialize requests
    # for this user
    ...
    # 1. Check Cooldown
    cooldown = UserOrganizedCrimeCooldown.query.filter_by(
        user_id=current_user.id, crime_id=crime_id).first()
    if cooldown:
        ...
        if ends_at > datetime.now(timezone.utc):
            ...
            flash(
                _(
                    'عليك الانتظار %(hours)s ساعة و %(minutes)s دقيقة قبل تكرار هذه الجريمة!',
                    hours=hours,
                    minutes=minutes),
                'danger')
    ...
    if existing_participation:
        ...
        return redirect(
            url_for(
                'main.lobby',
                lobby_id=existing_participation.lobby_id))
    ...
    min_creator_rank_level = int(SystemConfig.get_value(
        'organized_crimes_min_creator_rank_level', '20'))
    effective_level = current_user.level + (current_user.rank_points_value // 50) if hasattr(
        current_user, 'rank_points_value') else current_user.level
    ...
    # We'll assign the first role available as default, or "Leader" if not
    # defined.
...
def lobby(lobby_id):
    ...
    if _expire_lobby_if_needed(lobby):
        flash(
            _('انتهت صلاحية هذه العملية المنظمة لعدم اكتمال الفريق خلال 24 ساعة.'),
            'warning')
        return redirect(url_for('main.organized_crimes'))
    ...
    for role_def in roles_config:
        role_name = role_def.get('name') if isinstance(
            role_def, dict) else None
        participant = None
        if role_name:
            participant = next(
                (p for p in lobby.participants if p.role_name == role_name), None)
    ...
    current_participant = next(
        (p for p in lobby.participants if p.user_id == current_user.id), None)
    ...
    try:
        money_penalty = max(
            100, int(
                (lobby.crime.money_reward_min or 0) * 0.05))
    except BaseException:
        money_penalty = 100
    try:
    ...
    try:
        import json
        reqs = json.loads(
            lobby.crime.requirements) if lobby.crime.requirements else {}

        # Check for required item
        if 'required_item' in reqs:
            required_item_name = reqs['required_item']
            required_item_obj = Item.query.filter_by(
                name=required_item_name).first()
            if required_item_obj:
                required_item = required_item_obj
                # Check if anyone in lobby has it
                for p in lobby.participants:
                    user_has = UserItem.query.filter_by(
                        user_id=p.user_id).join(Item).filter(
                        Item.name == required_item_name).first()
                    if user_has:
...
def broadcast_lobby_invites(lobby_id):
    ...
    if last:
        ...
        if now - last < timedelta(hours=1):
            ...
            flash(
                _('تم إرسال الدعوات بالفعل. انتظر حوالي %(m)d دقيقة قبل التجديد.', m=minutes), 'info')
            return redirect(url_for('main.lobby', lobby_id=lobby.id))
    ...
    for u in recipients:
        try:
            send_notification(
                user_id=u.id,
                title=str(
                    _('دعوة للمشاركة في جريمة منظمة')),
                message=str(
                    _(
                        'القائد %(leader)s دعاك للمشاركة في "%(crime)s". سارع لاختيار دورك!',
                        leader=current_user.username,
                        crime=lobby.crime.name)),
                type='info',
                link=join_link)
        except Exception:
            continue
    ...
    flash(
        _('تم إرسال الدعوات لجميع اللاعبين. صلاحية الدعوات ساعة واحدة تقريباً.'),
        'success')
    return redirect(url_for('main.lobby', lobby_id=lobby.id))
...
def toggle_ready(lobby_id):
    ...
    participant = LobbyParticipant.query.filter_by(
        lobby_id=lobby.id, user_id=current_user.id).first()
    if not participant:
        abort(403)
    if lobby.status != 'recruiting':
        flash(_('المجموعة غير متاحة!'), 'warning')
...
def kick_member(lobby_id, user_id):
    ...
    participant = LobbyParticipant.query.filter_by(
        lobby_id=lobby.id, user_id=user_id).first()
    if not participant:
        abort(404)
...
def join_lobby(lobby_id):
    ...
    lobby = db.session.query(CrimeLobby).filter_by(
        id=lobby_id).with_for_update().first()
    if not lobby:
    ...
    cooldown = UserOrganizedCrimeCooldown.query.filter_by(
        user_id=current_user.id, crime_id=lobby.crime_id).first()
    if cooldown:
        ends_at = cooldown.cooldown_until
        if ends_at.tzinfo is None:
            ends_at = ends_at.replace(tzinfo=timezone.utc)
    ...
    roles_config = lobby.crime.roles_config or []
    max_slots = max(int(lobby.crime.min_members or 0), int(
        getattr(lobby.crime, 'max_members', 0) or 0), len(roles_config))
    ...
    if roles_config:
        valid_roles = [r.get('name')
                       for r in roles_config if isinstance(r, dict)]
    ...
    role_def = next((r for r in roles_config if isinstance(
        r, dict) and r.get('name') == role_name), None)
    if role_def:
...
def leave_lobby(lobby_id):
    ...
    if lobby.leader_id == current_user.id:
        ...
        try:
            energy_penalty = max(
                5, min(20, ((lobby.crime.energy_cost or 10) // 2)))
        except BaseException:
            energy_penalty = 10
        ...
        flash(_('انسحبت من المجموعة وتكبدت خسائر: -%(m)s$، -%(xp)s XP، -%(en)s طاقة.',
              m=money_penalty, xp=exp_penalty, en=energy_penalty), 'warning')
        return redirect(url_for('main.organized_crimes'))
...
def start_heist(lobby_id):
    ...
    for p in participants:
        ...
        if user.jail_until:
            ...
            if jail_until.tzinfo is None:
                jail_until = jail_until.replace(tzinfo=timezone.utc)
            if jail_until > now:
                flash(_('لا يمكن بدء العملية! العضو %(user)s في السجن.',
                      user=user.username), 'danger')
                return redirect(url_for('main.lobby', lobby_id=lobby.id))
        ...
        if user.hospital_until:
            ...
            if hospital_until.tzinfo is None:
                hospital_until = hospital_until.replace(tzinfo=timezone.utc)
            if hospital_until > now:
                flash(_('لا يمكن بدء العملية! العضو %(user)s في المستشفى.',
                      user=user.username), 'danger')
                return redirect(url_for('main.lobby', lobby_id=lobby.id))
        ...
        if user.gym_until:
            ...
            if gym_until.tzinfo is None:
                gym_until = gym_until.replace(tzinfo=timezone.utc)
            if gym_until > now:
                flash(_('لا يمكن بدء العملية! العضو %(user)s يتدرب في الجيم.',
                      user=user.username), 'danger')
                return redirect(url_for('main.lobby', lobby_id=lobby.id))
    ...
    for p in participants:
        role_def = next((r for r in roles_config if isinstance(
            r, dict) and r.get('name') == p.role_name), None)
        if role_def:
            min_stats = role_def.get('min_stats') or role_def.get('req') or {}
            for stat, req_val in min_stats.items():
                ...
                if user_val < req_val:
                    flash(
                        _(
                            'لا يمكن بدء العملية! العضو %(user)s لا يملك المهارات الكافية لدور %(role)s '
                            '(مطلوب %(stat)s: %(req)s).',
    ...
    if created_at.tzinfo is None:
        ...
        # If created_at is significantly in the future relative to UTC, it's
        # likely Local Time stored as Naive.
    ...
    if elapsed_seconds < planning_seconds:
        ...
        flash(_('يجب التخطيط جيداً! انتظروا %(sec)s ثانية للمراجعة قبل البدء.',
              sec=int(remaining)), 'warning')
        return redirect(url_for('main.lobby', lobby_id=lobby.id))
    ...
    try:
        if lobby.crime.requirements:
            ...
            if 'required_item' in reqs:
                ...
                for p in participants:
                    # Check UserItem
                    # Note: This query inside loop is not optimal but fine for
                    # small team size (max 4)
                    user_has = UserItem.query.filter_by(
                        user_id=p.user_id).join(Item).filter(
                ...
                if not has_item:
                    flash(
                        _(
                            'تحتاجون إلى "%(item)s" لتنفيذ هذه العملية! يمكنكم شراؤها من السوق السوداء.',
                            item=item_name),
                        'danger')
                    return redirect(url_for('main.lobby', lobby_id=lobby.id))
    except Exception:
    ...
    story_log = []
    story_log.append(
        _("بدأت عملية {crime} بقيادة {leader}...").format(
            crime=_(
                crime.name),
        ...
        if item and item.type == 'consumable':
            ...
            for p in participants:
                user_item = UserItem.query.filter_by(
                    user_id=p.user_id, item_id=item.id).with_for_update().first()
                if user_item and user_item.quantity > 0:
                    user_item.quantity -= 1
                    if user_item.quantity <= 0:
                        db.session.delete(user_item)
                    story_log.append(
                        _("تم استخدام {item} من قبل {user}.").format(
                            item=item.name, user=p.user.username))
                    break  # Deduct once per heist

    # Per-Role Item Deduction
    for p in participants:
        role_def = next(
            (r for r in roles_config if r['name'] == p.role_name), None)
        role_key = None
        ...
        if 'item' in role_reqs:
            ...
            if r_item and r_item.type == 'consumable':
                u_item = UserItem.query.filter_by(
                    user_id=p.user_id, item_id=r_item.id).with_for_update().first()
                if u_item and u_item.quantity > 0:
                    u_item.quantity -= 1
                    if u_item.quantity <= 0:
                        db.session.delete(u_item)
                    story_log.append(
                        _("استهلك {user} أداة {item} للدور.").format(
                            user=p.user.username, item=r_item.name))
    ...
    for p in participants:
        ...
        if 'intelligence' in min_stats:
            ...
            if diff >= 0:
                ...
                story_log.append(_("✅ {role} ({user}): أدى دوره بذكاء خارق! (+{bonus}%)").format(
                    role=_(p.role_name), user=user.username, bonus=5 + (diff // 10)))
            else:
                role_performance -= 10
                story_log.append(
                    _("⚠️ {role} ({user}): واجه صعوبات تقنية. (-10%)").format(
                        role=_(
                            p.role_name),
                        user=user.username))
        ...
        if 'strength' in min_stats:
            ...
            if diff >= 0:
                role_performance += 5 + (diff // 10)
                story_log.append(_("💪 {role} ({user}): سيطر على الوضع بقوة! (+{bonus}%)").format(
                    role=_(p.role_name), user=user.username, bonus=5 + (diff // 10)))
            else:
                role_performance -= 10
                story_log.append(
                    _("⚠️ {role} ({user}): لم يستطع كسر الدفاعات بسرعة. (-10%)").format(
                        role=_(
                            p.role_name),
                        user=user.username))
        ...
        if 'agility' in min_stats:
            ...
            if diff >= 0:
                role_performance += 5 + (diff // 10)
                story_log.append(_("🚗 {role} ({user}): قاد الفريق ببراعة وسرعة! (+{bonus}%)").format(
                    role=_(p.role_name), user=user.username, bonus=5 + (diff // 10)))
            else:
                role_performance -= 10
                story_log.append(
                    _("⚠️ {role} ({user}): تأخر في الوصول لنقطة الهروب. (-10%)").format(
                        role=_(
                            p.role_name),
                        user=user.username))
        ...
        if 'defense' in min_stats:
            ...
            if diff >= 0:
                ...
                story_log.append(_("🛡️ {role} ({user}): صدّ الضغط وحمى الفريق! (+{bonus}%)").format(
                    role=_(p.role_name), user=user.username, bonus=4 + (diff // 10)))
            else:
                role_performance -= 8
                story_log.append(
                    _("⚠️ {role} ({user}): انهك من الضغط وتباطأ. (-8%)").format(
                        role=_(
                            p.role_name),
                        user=user.username))
    ...
    driver_participant = next(
        (p for p in participants if 'driver' in p.role_name.lower() or 'سائق' in p.role_name),
        None)

    if driver_participant:
        # Check their active vehicle
        active_vehicle = UserVehicle.query.filter_by(
            user_id=driver_participant.user_id, is_active=True).first()
        if active_vehicle:
            ...
            story_log.append(
                _("🏎️ السائق {user} يستخدم سيارة {car} المعدلة! (+{bonus}%)").format(
            ...
            story_log.append(
                _("⚠️ السائق {user} لا يملك سيارة مجهزة! الهروب سيكون صعباً. (-10%)").format(
                    user=driver_participant.user.username))
    else:
        ...
        for p in participants:
            active_vehicle = UserVehicle.query.filter_by(
                user_id=p.user_id, is_active=True).first()
            if active_vehicle:
        ...
        if best_vehicle_bonus > 0:
            ...
            story_log.append(
                _("🚗 الفريق يستخدم سيارة {user} ({car}) للهروب! (+{bonus}%)").format(
                    user=owner_name,
                    car=vehicle_name,
                    bonus=best_vehicle_bonus))
    ...
    if chaos_roll <= 5:
        ...
        for p in participants:
            if random.random() < 0.3:  # 30% chance of injury
                ...
                story_log.append(
                    _("🚑 {user} أصيب بطلق ناري! تم نقله للمستشفى.").format(
                        user=p.user.username))
    ...
    try:
        import json
        reqs_heat = json.loads(
            lobby.crime.requirements) if lobby.crime.requirements else {}
        heat_window = int(reqs_heat.get('heat_window_hours', 6))
        heat_success_penalty = float(
            reqs_heat.get(
                'heat_success_penalty_pct', 5.0))
        ...
        for p in participants:
            ...
            if lc:
                ...
                if (now_ts - lc) <= timedelta(hours=heat_window):
                    recent_ops += 1
        if recent_ops > 0:
            success_penalty_total = heat_success_penalty * \
                min(recent_ops, len(participants))
            success_probability = max(
                5, success_probability - success_penalty_total)
            story_log.append(
                _("🚨 حرارة الساحة مرتفعة: فرص النجاح انخفضت بنسبة {pct}%").format(
                    pct=int(success_penalty_total)))
    except Exception:
        pass
    try:
        import json
        reqs_patrol = json.loads(
            lobby.crime.requirements) if lobby.crime.requirements else {}
        dp_chance = float(reqs_patrol.get('double_patrol_pct', 3.0))
        dp_success_penalty = float(reqs_patrol.get(
            'double_patrol_success_penalty_pct', 7.5))
        dp_jail_boost = float(
            reqs_patrol.get(
                'double_patrol_jail_boost_pct',
                15.0))
        double_patrol_active = False
        if random.random() * 100 <= dp_chance:
            success_probability = max(
                5, success_probability - dp_success_penalty)
            double_patrol_active = True
            story_log.append(
                _("🚓 دورية مزدوجة في المنطقة! انخفضت فرص النجاح وزادت المخاطر."))
    except Exception:
        double_patrol_active = False
    roll = random.randint(1, 100)
    ...
    story_log.append(
        _("--- النتيجة النهائية: نسبة النجاح {prob}% (الرقم العشوائي: {roll}) ---").format(
            prob=success_probability, roll=roll))
    ...
    if is_success:
        ...
        try:
            ...
            if random.random() * 100 <= lucky_chance:
                reward_money = int(
                    reward_money * (1 + lucky_money_pct / 100.0))
        ...
        story_log.append(
            _("💰 نجحت العملية! الغنيمة الكلية: {money} شيكل.").format(
                money=reward_money))
        ...
        if participant_performance:
            mvp_user_id = max(
                participant_performance,
                key=participant_performance.get)

        for p in participants:
            # Skip if hospitalized (no reward if unconscious? No, let's give
            ...
            try:
                ...
                if money_pct or exp_bonus:
                    story_log.append(_("🏅 دور {role} منح مكافأة خاصة لعضو: +{m} مال، +{e} خبرة").format(
                        role=p.role_name, m=int(share * (money_pct / 100.0)), e=exp_bonus))
                ...
                try:
                    recent_histories = HeistHistory.query.order_by(
                        HeistHistory.created_at.desc()).limit(10).all()
                    streak = 0
                    for h in recent_histories:
                        names = [
                            ps.get('name') for ps in (
                                h.participants_snapshot or [])]
                    ...
                    if streak > 0:
                        ...
                        se = int((reward_exp + bonus_exp)
                                 * (streak_pct / 100.0))
                        bonus_money += sm
                        bonus_exp += se
                        story_log.append(
                            _("🔥 سلسلة انتصارات: مكافآت إضافية بنسبة {pct}% للاعب {user}").format(
                                pct=int(streak_pct), user=p.user.username))
                except Exception:
                    pass
            except Exception:
                pass
            ...
            if p.user_id == mvp_user_id and len(participants) > 1:
                ...
                story_log.append(
                    _("🌟 {user} كان الأفضل أداءً! (+{m} مال، +{e} خبرة)").format(
                        user=p.user.username, m=mvp_bonus_money, e=mvp_bonus_exp))
            ...
            if p.user_id == lobby.leader_id:
                ...
                story_log.append(
                    _("👑 القائد {user} حصل على مكافأة القيادة: {bonus} شيكل.").format(
                        user=p.user.username, bonus=leader_bonus))
            ...
            cooldown_rec = UserOrganizedCrimeCooldown.query.filter_by(
                user_id=p.user_id, crime_id=crime.id).first()
            if not cooldown_rec:
            ...
            # ResourceService.modify_resources uses with_for_update() for
            # locking.

            # Add last_crime to set_fields to ensure atomic update
            set_fields_dict['last_crime'] = datetime.now(
            ...
            # Refresh user object to reflect DB changes made by ResourceService
            # (important for check_level_up)
            ...
            try:
                ...
                progress = UserProgress.query.filter_by(
                    user_id=p.user_id).with_for_update().first()
                if progress:
        ...
        for p in participants:
            ...
            if p.user_id in injuries:
                ...
                # Already hospitalized before heist (should not happen due to
                # check, but safe to keep)
                status = 'hospitalized'
            ...
            if status != 'hospitalized':
                ...
                if active_vehicle:
                    ...
                    vehicle_escape_bonus = min(
                        0.5, active_vehicle.vehicle.price / 200000.0)
                ...
                if random.random() < min(0.9, max(0.05, jail_chance)):
                    ...
                    jail_until_dt = (
                        datetime.now(
                            timezone.utc) +
                        timedelta(
                            minutes=jail_time)).replace(
                        tzinfo=None)
                    set_fields_dict['jail_until'] = jail_until_dt
                    story_log.append(
                        _("👮 {user} تم القبض عليه! ({time} دقيقة سجن)").format(
                            user=p.user.username, time=jail_time))
                else:
                    ...
                    if p.user.id == current_user.id:
                        ...
                        story_log.append(
                            _("🚔 {user} الشرطة تطاردك! استعد للهروب!").format(
                                user=p.user.username))
                    else:
                        story_log.append(
                            _("🏃 {user} تمكن من الهروب بأعجوبة.").format(
                                user=p.user.username))
...
def heist_report(history_id):
    ...
    if any('🚗' in (line or '') or '🏎️' in (line or '') for line in lines):
        animation = 'speed'

    return render_template(
        'heist_report.html',
        history=history,
        crime_image=crime_image,
        lines=lines,
        animation=animation)
...
def heist_history():
    # Show last 20 heists
    history = HeistHistory.query.order_by(
        HeistHistory.created_at.desc()).limit(20).all()
