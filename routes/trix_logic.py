...
class TrixGameLogic:
    ...
    @staticmethod
    def deal(state):
        deck = [{'suit': s, 'rank': r} for s in TrixGameLogic.SUITS for r in TrixGameLogic.RANKS]
        random.shuffle(deck)
        for i in range(4):
            hand = deck[i*13:(i+1)*13]
            hand.sort(key=lambda c: (TrixGameLogic.SUITS.index(c['suit']), TrixGameLogic.RANK_VALUES[c['rank']]))
    ...
    def play_trick_card(state, player_index, card):
        ...
        card_in_hand = next((c for c in hand if c['suit'] == card['suit'] and c['rank'] == card['rank']), None)
        
        if not card_in_hand:
            return {'valid': False, 'message': 'Card not in hand'}
             
        if not state['trick'] and card['suit'] == '♥' and state.get('current_contract') == 'king':
            has_non_hearts = any(c['suit'] != '♥' for c in hand)
            if has_non_hearts and not state.get('hearts_broken'):
                return {'valid': False, 'message': 'Cannot lead hearts unless only hearts remain'}
        ...
        if state['trick']:
            ...
            if card['suit'] != lead_suit:
                ...
                if has_suit:
                    try:
                        hand_str = " ".join([f"{h['suit']}{h['rank']}" for h in hand])
                        logger.debug(f"Trix follow-suit violation seat={player_index} lead={lead_suit} attempted={card} hand={hand_str}")
                    except Exception:
                        pass
    ...
    def resolve_trick(state):
        ...
        if contract == 'king':
            ...
            for c in cards:
                if c['suit'] == '♥' and c['rank'] == 'K':
                    state['scores'][winner_player] += (-150 if doubles.get('king') is not None else -75)
                if c['rank'] == 'Q':
                    mult = -50 if c['suit'] in doubles.get('queens', {}) else -25
    ...
    def play_trix_card(state, player_index, card):
        ...
        if card.get('action') == 'pass':
            if TrixGameLogic.has_valid_trix_move(hand, state['trix_piles']):
                return {'valid': False, 'message': 'You have valid moves, cannot pass'}
            else:
                state['turn_seat'] = (state['turn_seat'] + 1) % 4
                return {'valid': True, 'state': state}
    ...
    def declare_double(state, player_index, dtype, suit=None):
        ...
        if dtype == 'king':
            ...
            if not has_king:
                return {'valid': False, 'message': 'You must hold King of Hearts to double'}
            ...
            if not has_q:
                return {'valid': False, 'message': 'You must hold this Queen to double'}
