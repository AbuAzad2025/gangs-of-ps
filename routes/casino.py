...
def blackjack_index():
    ...
    game = CasinoGame.query.filter_by(
    ...
    # We can check for recently completed game to show result if we want, but
    # flash is enough.
...
def blackjack_deal():
    ...
    # Check existing game
    active_game = CasinoGame.query.filter_by(
        user_id=current_user.id,
        game_type='blackjack',
        status='active').first()
    if active_game:
    ...
    if player_score == 21:
        ...
        game_state['message'] = _(
            'بلاك جاك! ربحت %(amount)s!',
            amount=winnings)
...
def blackjack_stand():
    ...
    if dealer_score > 21:
        ...
        # Legacy support
            ...
            current_user.casino_luck_until
            and current_user.casino_luck_until.replace(tzinfo=timezone.utc) > datetime.now(timezone.utc)
        ):
        ...
        if current_user.active_hostess_id:
            ...
            if hostess and hostess.buff_type == 'casino_luck':
                chance = 0.2 + \
                    (hostess.buff_value if hostess.buff_value else 0)
                if random.random() < chance:
                    saved_by_hostess = True

        if not saved_by_hostess and current_user.casino_luck_until and current_user.casino_luck_until.replace(
...
def roulette_spin():
    ...
    if won:
        ...
        trigger_hostess_reaction(current_user, winnings, True)
        message = _(
            'الكرة وقفت على %(num)s (%(col)s). مبروك! ربحت %(win)s!',
            num=landing,
            col=color,
            win=winnings)
        ...
        if current_user.casino_luck_until and current_user.casino_luck_until.replace(
            ...
            if random.random() < 0.2:
                ...
                message = _(
                    'الكرة وقفت على %(num)s. لكن المضيفة عوضتك بـ %(ref)s!',
                    num=landing,
                    ref=refund)
        ...
        if not saved:
            ...
            message = _(
                'الكرة وقفت على %(num)s (%(col)s). حظاً أوفر!',
                num=landing,
                col=color)
...
def bar():
    drinks = [{'id': 1,
              ...
              {'id': 2,
               ...
               'brave': 1,
               'health': 0,
               'desc': _('منعشة في هذا الجو الحار')},
              {'id': 3,
               'name': _('فودكا روسي'),
               'cost': 500,
               'energy': 40,
               ...
               'desc': _('قوية جداً!')},
              {'id': 4,
               'name': _('كوكتيل الزعيم'),
               'cost': 1500,
               'energy': 100,
               'brave': 10,
               'health': 10,
               'desc': _('المشروب المفضل للزعماء. يعيد طاقتك بالكامل!')}]
    return render_template(
        'casino/bar.html',
        drinks=drinks,
        user=current_user,
...
def buy_drink(drink_id):
    ...
    effect_changes = {}
    if energy_gain != 0:
        effect_changes['energy'] = energy_gain
    if brave_gain != 0:
        effect_changes['brave'] = brave_gain
    if health_delta != 0:
        effect_changes['health'] = health_delta

    if effect_changes:
        ResourceService.modify_resources(
            user.id,
            effect_changes,
            f'casino_bar_effect_{drink_id}',
            auto_commit=False)
...
def hostess():
    ...
    active_hostesses = Hostess.query.filter(
        Hostess.current_player_id is not None).all()
    ...
    hostesses_list = Hostess.query.filter_by(
        is_active=True, is_public=False).order_by(
        Hostess.price.asc()).all()
    ...
    if current_user.active_hostess_id:
        active_h = db.session.get(
            Hostess, current_user.active_hostess_id)  # Fetch fresh
        if active_h:
...
def hire_hostess(h_id):
    ...
    if current_user.level < h.min_rank:
        flash(_('مستواك لا يسمح بالجلوس مع هذه المضيفة! (مطلوب مستوى %(lvl)s)',
              lvl=h.min_rank), 'warning')
        return redirect(url_for('casino.hostess'))
    ...
    if not ResourceService.modify_resources(
            current_user.id, {
            'money': -h.price}, 'hire_hostess', auto_commit=False, expected_version=None):
        flash(_('ما معك كاش يكفي لجلوس مع هذه الجميلة!'), 'danger')
        return redirect(url_for('casino.hostess'))
    ...
    if current_user.casino_luck_until:
        ...
        if luck_until > now:
            current_user.casino_luck_until = luck_until + \
    ...
    db.session.commit()
    flash(_('تم التعاقد مع %(name)s! ستكون بجانبك لمدة %(min)s دقيقة.',
          name=h.name, min=duration), 'success')
    return redirect(url_for('casino.hostess'))
...
def chat_hostess():
    ...
    if not current_user.casino_luck_until:
        return jsonify(
            {'response': _('انتهى وقتنا يا عزيزي. جدد العقد اذا اردت المزيد.')})
