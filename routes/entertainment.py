...
def create_room():
    ...
    allowed_curr = (SystemConfig.get_value('betting_allowed_currencies', 'money,diamonds') or 'money,diamonds').split(',')
    ...
    if game_enabled_key and SystemConfig.get_value(game_enabled_key, 'true') != 'true':
    ...
    try:
        min_stake = int(SystemConfig.get_value('betting_min_stake', '0') or '0')
    except Exception:
        min_stake = 0
    try:
        max_stake = int(SystemConfig.get_value('betting_max_stake', '1000000000') or '1000000000')
    except Exception:
    ...
    if stake_amount > 0:
        ...
        try:
            ...
            # Fallback for DBs that don't support with_for_update or if it fails
    ...
    if mode == 'solo':
        ...
        if game_type == 'chess':
            room.game_state = {'fen': chess.STARTING_FEN, 'history': [], 'turn': 'w', 'is_solo': True, 'engine': 'minimax'}
            if stake_amount > 0:
                room.pot_amount += stake_amount # 1 Bot
        elif game_type == 'trix':
            room.game_state = {'is_solo': True, 'bot_seats': [1, 2, 3], 'trix_style': trix_style, 'team_mode': trix_team_mode}
            ...
            room.game_state = {'is_solo': True, 'bot_seats': [1, 2, 3], 'team_mode': 'partnership'}
        ...
        if game_type == 'chess':
            room.game_state = {'fen': chess.STARTING_FEN, 'history': [], 'turn': 'w'}
        elif game_type == 'trix':
            room.game_state = {'phase': 'lobby', 'players_ready': [], 'trix_style': trix_style, 'team_mode': trix_team_mode}
        elif game_type == 'tarneeb':
            room.game_state = {'phase': 'lobby', 'players_ready': [], 'team_mode': 'partnership'}
...
def room(room_id):
    ...
    if not player:
        ...
        try:
            ...
            r_lock = db.session.query(GameRoom).filter_by(id=room.id).with_for_update().first()
            if r_lock:
    ...
    if room.status == 'playing':
        if room.game_type == 'chess':
            return render_template('entertainment/chess_online.html', room=room, player=player)
        elif room.game_type == 'trix':
            return render_template('entertainment/trix_online.html', room=room, player=player)
        elif room.game_type == 'tarneeb':
            return render_template('entertainment/tarneeb_online.html', room=room, player=player)
            
    return render_template('entertainment/lobby_room.html', room=room, player=player)

def _chess_piece_value(piece_type):
    if piece_type == chess.PAWN: return 100
    if piece_type == chess.KNIGHT: return 320
    if piece_type == chess.BISHOP: return 330
    if piece_type == chess.ROOK: return 500
    if piece_type == chess.QUEEN: return 900
    if piece_type == chess.KING: return 0
    return 0
...
def _chess_evaluate(board: chess.Board):
    ...
    for piece_type in [chess.PAWN, chess.KNIGHT, chess.BISHOP, chess.ROOK, chess.QUEEN]:
        score += len(board.pieces(piece_type, chess.WHITE)) * _chess_piece_value(piece_type)
        score -= len(board.pieces(piece_type, chess.BLACK)) * _chess_piece_value(piece_type)
...
def _chess_best_move(board: chess.Board, max_depth: int = 2):
    ...
    white_material = sum(_chess_piece_value(pt) * len(board.pieces(pt, chess.WHITE)) for pt in [chess.PAWN, chess.KNIGHT, chess.BISHOP, chess.ROOK, chess.QUEEN])
    black_material = sum(_chess_piece_value(pt) * len(board.pieces(pt, chess.BLACK)) for pt in [chess.PAWN, chess.KNIGHT, chess.BISHOP, chess.ROOK, chess.QUEEN])
...
def get_room_state(room_id):
    ...
    if room.game_type != 'trix':
        messages = room.messages.order_by(GameChat.created_at.asc()).limit(50).all()
        data['messages'] = [m.to_dict() for m in messages]
    
    # Solo game enhancements: drive bot turns and expose bot players
    try:
        if room.game_type == 'chess' and room.game_state and room.game_state.get('is_solo'):
            ...
            if state.get('is_solo') or len(state.get('bot_seats', [])) > 0:
                ...
                if room.game_type == 'tarneeb' and state.get('phase') == 'bidding':
                    if not state.get('current_bid', {}).get('bidder'):
                        ...
                        if turn != 0:
                            try:
                                hand = state.get('hands', [[] for _ in range(4)])[turn]
                                suit_counts = {s: 0 for s in TarneebGameLogic.SUITS}
                                for c in hand:
                                    suit_counts[c['suit']] += 1
                                best_suit = max(suit_counts, key=lambda s: suit_counts[s])
                                state['current_bid'] = {'value': 7, 'trump': best_suit, 'bidder': turn}
                                state['bidding_history'] = state.get('bidding_history', [])
                                state['bidding_history'].append({'player': turn, 'action': 'bid', 'value': 7, 'trump': best_suit})
                ...
                # Check for finish
                if state.get('phase') == 'finished' and room.status != 'finished':
...
def toggle_ready(room_id):
    ...
    try:
        room = db.session.query(GameRoom).filter_by(id=room_id).with_for_update().first()
    except Exception:
        room = GameRoom.query.get_or_404(room_id)
        
    if not room: abort(404)

    player = GamePlayer.query.filter_by(room_id=room.id, user_id=current_user.id).first_or_404()
...
def start_room(room_id):
    ...
    if room.status != 'playing':
        ...
        # Check for missing players and fill with bots if necessary (Hybrid Mode)
        players_count = room.players.count()
        
        if room.game_type == 'trix':
            if players_count < 4:
                existing_seats = [p.seat_index for p in room.players.all()]
                missing_seats = [i for i in range(4) if i not in existing_seats]
...
def delete_room(room_id):
    ...
    if room.status != 'finished' and room.stake_amount > 0:
        ...
        for p in players:
            ...
            try:
                u = db.session.query(User).filter_by(id=p.user_id).with_for_update().first()
            except Exception:
    ...
    if socketio:
        try:
            socketio.emit('room_update', {'deleted': True, 'room_id': room_id}, room=f'room-{room_id}')
        except Exception:
            pass
    return jsonify({'status': 'ok'})
...
def leave_game(room_id):
    ...
    if room.status == 'playing':
        ...
        if room.game_type == 'chess':
            ...
            if room.game_state:
                state = dict(room.game_state) # Copy to ensure change detection
            ...
            # Notify room to refresh state (which will pick up missing seat as bot)
            if socketio:
                payload = room.to_dict()
                socketio.emit('room_update', payload, room=f'room-{room.id}')
    ...
    def event_stream():
        for _ in range(300):
            ...
            if not room:
                ...
                break
            payload = room.to_dict()
            # Enrich players for solo games with bot seats and drive bot turns lightly
            if room.game_state and room.game_state.get('is_solo') and room.game_type in ['trix', 'tarneeb']:
                state = room.game_state
    ...
    if room.game_type == 'chess':
        ...
        if action == 'set_engine':
            ...
            if not current_state.get('is_solo'):
                return jsonify({'error': 'Engine toggle only in solo mode'}), 400
            ...
            if current_state.get('engine') != 'client_stockfish':
                return jsonify({'error': 'Engine not set to client_stockfish'}), 400
            try:
            ...
            if (board.turn == chess.WHITE and human_is_white) or (board.turn == chess.BLACK and not human_is_white):
                return jsonify({'error': 'Engine cannot move on human turn'}), 403
            ...
            try:
                ...
                if move in board.legal_moves:
                    board.push(move)
                    current_state['fen'] = board.fen()
                    # current_state['turn'] is derived from FEN, but we can keep explicit track if needed
                    current_state['turn'] = 'w' if board.turn == chess.WHITE else 'b'
        ...
        if action == 'choose_contract':
            if current_state['phase'] != 'choose_contract':
                return jsonify({'error': 'Not in contract selection phase'}), 400
            
            if current_state['kingdom_player'] != player.seat_index:
                 return jsonify({'error': 'Not your turn to choose contract'}), 403
                 
            result = TrixGameLogic.start_contract(current_state, data.get('contract'))
            ...
            suit = data.get('suit')
            try:
                result = TrixGameLogic.declare_double(current_state, player.seat_index, dtype, suit)
                if not result.get('valid'):
                    return jsonify({'error': result.get('message')}), 400
                current_state = result['state']
            except Exception as e:
            ...
            try:
                result = TrixGameLogic.confirm_doubling(current_state, player.seat_index)
                if not result.get('valid'):
                    return jsonify({'error': result.get('message')}), 400
                current_state = result['state']
            except Exception as e:
                current_app.logger.exception(f"Trix confirm_doubling error: {e}")
                return jsonify({'error': 'Server error during confirm'}), 500
            
        elif action == 'play_card':
            current_app.logger.debug(f"Trix Play Card Request seat={player.seat_index} card={data.get('card')}")
            try:
                result = TrixGameLogic.play_card(current_state, player.seat_index, data.get('card'))
                current_app.logger.debug(f"Trix Play Card Result valid={result.get('valid')} msg={result.get('message')}")
            ...
            try:
                while True:
                    if current_state['phase'] == 'choose_contract':
                        ...
                        if king_seat != 0:
                            import random
                            available = current_state.get('available_contracts', [])
                            if available:
                                contract = random.choice(available)
                                res = TrixGameLogic.start_contract(current_state, contract)
                                if res.get('valid'):
                        ...
                        for b in bot_indices:
                            bot_move = TrixGameLogic.get_bot_move(current_state, b)
                            if bot_move:
                                res = None
                                if bot_move['type'] == 'confirm_doubling':
                                    res = TrixGameLogic.confirm_doubling(current_state, b)
                                elif bot_move['type'] == 'double':
                                    res = TrixGameLogic.declare_double(current_state, b, bot_move['item'], bot_move.get('suit'))
                        ...
                        if current_state['turn_seat'] != 0:
                            ...
                            bot_move = TrixGameLogic.get_bot_move(current_state, bot_seat)
                            if bot_move and bot_move.get('type') == 'play':
                                res = TrixGameLogic.play_card(current_state, bot_seat, bot_move['card'])
                                if res.get('valid'):
                                    current_state = res['state']
                                    if current_state.get('phase') == 'finished':
                                        room.status = 'finished'
                            ...
                            break
                    if current_state.get('phase') not in ['choose_contract', 'playing', 'doubling']:
                        break
            except Exception as e:
        ...
        if action == 'bid':
            try:
                res = TarneebGameLogic.make_bid(current_state, player.seat_index, data.get('bid', {}))
                if not res.get('valid'):
                    return jsonify({'error': res.get('message')}), 400
                current_state = res['state']
                # Safety nudge: if human passed and bots haven't started, seed minimal bid
                if current_state.get('phase') == 'bidding':
                    if not current_state.get('current_bid', {}).get('bidder'):
                        turn = current_state.get('turn_seat', 0)
                        if turn != 0:
                            try:
                                hand = current_state.get('hands', [[] for _ in range(4)])[turn]
                                suit_counts = {s: 0 for s in TarneebGameLogic.SUITS}
                                for c in hand:
                                    suit_counts[c['suit']] += 1
                                best_suit = max(suit_counts, key=lambda s: suit_counts[s])
                                current_state['current_bid'] = {'value': 7, 'trump': best_suit, 'bidder': turn}
                                hist = current_state.get('bidding_history', [])
                                hist.append({'player': turn, 'action': 'bid', 'value': 7, 'trump': best_suit})
            ...
            try:
                res = TarneebGameLogic.handle_doubling(current_state, player.seat_index, data.get('doubling', {}))
                if not res.get('valid'):
                    return jsonify({'error': res.get('message')}), 400
                current_state = res['state']
            except Exception as e:
            ...
            try:
                res = TarneebGameLogic.play_card(current_state, player.seat_index, data.get('card'))
                if not res.get('valid'):
                    return jsonify({'error': res.get('message')}), 400
                current_state = res['state']
                if current_state.get('phase') == 'finished':
        ...
        if current_state.get('is_solo') or len(current_state.get('bot_seats', [])) > 0:
            print(f"[DEBUG] Entering bot loop. Phase: {current_state.get('phase')}, Turn: {current_state.get('turn_seat')}")
            steps = 0
            while steps < 8:
                steps += 1
                if current_state.get('phase') == 'bidding':
                    ...
                    if turn != 0:
                        action = TarneebGameLogic.get_bot_action(current_state, turn)
                        print(f"[DEBUG] Bot {turn} action: {action}")
                        if action['type'] in ['bid', 'pass']:
                            res = TarneebGameLogic.make_bid(current_state, turn, action['bid'])
                            if res.get('valid'):
                                current_state = res['state']
                                continue
                            else:
                    ...
                    if turn != 0:
                        action = TarneebGameLogic.get_bot_action(current_state, turn)
                        if action['type'] == 'doubling':
                            res = TarneebGameLogic.handle_doubling(current_state, turn, action['doubling'])
                            if res.get('valid'):
                                current_state = res['state']
                                continue
                            else:
                    ...
                    if turn != 0:
                        action = TarneebGameLogic.get_bot_action(current_state, turn)
                        if action['type'] == 'play':
                            res = TarneebGameLogic.play_card(current_state, turn, action['card'])
                            if res.get('valid'):
                                current_state = res['state']
                                if current_state.get('phase') == 'finished':
                                    room.status = 'finished'
...
def chess_solo():
    ...
    room = GameRoom(game_type='chess', name=f"{current_user.username}'s Solo Game", status='playing')
    room.game_state = {'fen': chess.STARTING_FEN, 'history': [], 'turn': 'w', 'is_solo': True} 
...
def trix():
    ...
    room = GameRoom(game_type='trix', name=f"{current_user.username}'s Solo Game", status='playing')
