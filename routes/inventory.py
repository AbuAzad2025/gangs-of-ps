...
def index():
    items = UserItem.query.filter_by(
        ...
        UserItem.quantity > 0).limit(100).all()
...
def equip(item_id):
    ...
    user_item = db.session.query(UserItem).filter_by(
        id=item_id, user_id=current_user.id).with_for_update().first()
    if not user_item:
        abort(404)
...
def repair(item_id):
    ...
    # 2. Lock Item
    # and ensure consistent lock order (User -> Item) if other routes follow
    # this.
    user_item = db.session.query(UserItem).filter_by(
        id=item_id, user_id=current_user.id).with_for_update().first()
    ...
    # 3. Call ResourceService (it will re-lock User, which is fine as we hold
    # the lock)
    if not ResourceService.modify_resources(
            current_user.id, {
            'money': -real_cost}, 'repair_item', auto_commit=False, expected_version=None):
        db.session.rollback()
    ...
    flash(_('تم إصلاح %(name)s مقابل %(cost)s$.',
          name=user_item.item.name, cost=real_cost), 'success')
    return redirect(url_for('inventory.index'))
