...
from models import Asset, Gang, MoneySinkLog, GangLog, User
...
def collect_income(asset_id):
    ...
    # Lock User first to prevent race conditions (Consistent Order: User ->
    # Asset)
    ...
    asset = db.session.query(Asset).filter_by(
        id=asset_id).with_for_update().first()
    ...
    if maintenance > 0:
        ...
        flash(_('تم جمع الدخل %(amount)s $ من %(name)s بنجاح!',
              amount="{:,}".format(asset.income), name=asset.name), 'success')
    return redirect(url_for('economy.my_properties'))
...
def collect_gang_income(asset_id):
    ...
    try:
        ...
        if gang_buff > 0:
            ...
            net_income += bonus
            if bonus > 0:
                flash(_('مكافأة ملوك الشوارع: +%(bonus)s$',
                      bonus="{:,}".format(bonus)), 'success')
    except Exception:
        pass
