...
class Gang(db.Model):
    ...
    leader_id = db.Column(
        ...
        db.ForeignKey(
            'user.id',
            use_alter=True,
            name='fk_gang_leader_id'),
        nullable=False,
        index=True)
    underboss_id = db.Column(
        db.Integer,
        db.ForeignKey(
            'user.id',
            use_alter=True,
            name='fk_gang_underboss_id'),
        nullable=True,
        index=True)
    ...
    recruitment_status = db.Column(db.String(20),
                                   default='open')  # open, invite_only, closed
    allowed_countries = db.Column(db.String(255),
                                  nullable=True)  # Comma separated ISO codes
    ...
    members = db.relationship(
        ...
        backref='gang',
        lazy=True,
        foreign_keys='User.gang_id')
    leader = db.relationship(
        'User',
        foreign_keys=[leader_id],
        backref='leader_of_gang')
...
class GangLog(db.Model):
    ...
    user_id = db.Column(
        ...
        db.ForeignKey('user.id'),
        nullable=True,
        index=True)  # Who performed the action
...
class GangWar(db.Model):
    ...
    war_type = db.Column(
        db.String(50),
        default='street')  # street, cyber, economic
    winner_id = db.Column(
        db.Integer,
        db.ForeignKey('gang.id'),
        nullable=True,
    ...
    gang1 = db.relationship(
        'Gang',
        foreign_keys=[gang1_id],
        backref='wars_started')
    gang2 = db.relationship(
        'Gang',
        foreign_keys=[gang2_id],
        backref='wars_received')
...
class GangAlliance(db.Model):
    ...
    gang1 = db.relationship(
        'Gang',
        foreign_keys=[gang1_id],
        backref='alliances_initiated')
    gang2 = db.relationship(
        'Gang',
        foreign_keys=[gang2_id],
        backref='alliances_received')
...
class PublicChat(db.Model):
    ...
    def to_dict(self):
        ...
        is_guest = bool(role == UserRole.GUEST) if role is not None else False
        is_vip = bool(role and role.value >=
                      UserRole.SUBSCRIBER.value) if role is not None else False
        return {
            'id': self.id,
...
class GangChat(db.Model):
    __tablename__ = 'gang_chat'
    id = db.Column(db.Integer, primary_key=True)
    gang_id = db.Column(
        db.Integer,
        db.ForeignKey('gang.id'),
    ...
    gang = db.relationship('Gang', backref='chat_messages')
    user = db.relationship('User', backref='gang_chats')
...
class PrivateChat(db.Model):
    ...
    sender = db.relationship(
        ...
        backref='sent_private_chats')
    receiver = db.relationship(
        'User',
        foreign_keys=[receiver_id],
        backref='received_private_chats')

    __table_args__ = (
        db.Index('idx_private_chat_conversation', 'sender_id', 'receiver_id'),
    )
    ...
    def to_dict(self):
        return {
            ...
            'created_at': self.created_at.strftime('%H:%M'),
            'timestamp_iso': self.created_at.isoformat()
        }
...
class Friendship(db.Model):
    ...
    def other_user_id(self, current_user_id: int) -> int:
        if int(current_user_id) == int(self.user1_id):
            return int(self.user2_id)
        return int(self.user1_id)
