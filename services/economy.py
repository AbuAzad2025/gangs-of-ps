...
def apply_daily_sinks():
    ...
    # Optimization: Fetch IDs first to avoid loading all User objects into
    # memory
    user_ids = [
        r[0] for r in db.session.query(
            User.id).filter(
            User.bank_balance >= config["tier1_threshold"]).all()]
    ...
    current_app.logger.info(
        f"Applied Bank Fees: {total_fees} from {count} users.")
    ...
    try:
        ...
        facility_ids = [
            r[0] for r in db.session.query(
                UserFacility.id).filter(
                UserFacility.level > 0).all()]
        ...
        for fid in facility_ids:
            try:
                ...
                current_app.logger.error(
                    f"Error processing maintenance for facility {fid}: {e}")
                db.session.rollback()

        # Removed bulk commit
        current_app.logger.info(
            f"Applied Maintenance: {maint_total} from {maint_count} facilities.")
...
def create_daily_snapshot():
    ...
    if existing:
        current_app.logger.info(
            f"Snapshot for {today} already exists. Updating...")
        snapshot = existing
    else:
    ...
    snapshot.avg_wealth = int(
        total_wealth /
        user_count) if user_count > 0 else 0
    ...
    if user_count > 0:
        ...
        top_users = User.query.with_entities(
            User.money +
            User.bank_balance).order_by(
            (User.money +
             User.bank_balance).desc()).limit(top_count).all()
        top_wealth = sum([u[0] for u in top_users])

        snapshot.top_1_percent_share = (
            top_wealth / total_wealth * 100) if total_wealth > 0 else 0
    ...
    if snapshot.top_1_percent_share > 45.0:
        ...
        if current_pct < 0.025:  # Cap at 2.5%
            ...
            SystemConfig.set_value(
                "bank_fee_tier2_pct",
                str(new_pct),
                "Auto-adjusted due to high inequality")
