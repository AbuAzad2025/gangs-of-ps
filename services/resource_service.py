    ...
    def modify_resources(user_id, changes, reason, check_balance=True, auto_commit=True, expected_version=None, set_fields=None, log_extra=None):
        ...
        try:
            ...
            if 'money' in changes and changes['money'] > 0:
                ...
                # However, to avoid capping P2P transfers and destroying economy, we only cap Faucets.
                CAPPED_REASONS = [
                ...
                if reason in CAPPED_REASONS:
                    try:
                        ...
                        limit = int(SystemConfig.get_value('economy_daily_money_limit', '1000000'))
                        current_daily = user.daily_money_earned or 0
                        
                        if current_daily >= limit:
                            changes['money'] = 0 # Cap reached
                            try:
                                flash(_('لقد وصلت للحد اليومي للكسب (%(limit)s$)! لن تكسب المزيد اليوم.', limit=limit), 'warning')
                            except:
                                pass
                            ...
                            try:
                                flash(_('لقد وصلت للحد اليومي للكسب! تم إضافة %(allowed)s$ فقط.', allowed=allowed), 'warning')
                            except:
                                pass
                        else:
