...
class MarketSimulationService:
    ...
    def get_asset_config():
        base = {}
        for sym, cfg in (
                MarketSimulationService.BASE_ASSET_CONFIG or {}).items():
            base[sym] = dict(cfg)
        ...
        if raw:
            ...
            if isinstance(data, dict):
                for k, v in data.items():
                    ...
                    if not sym or not re.fullmatch(
                            r'[A-Z0-9][A-Z0-9-]{0,9}', sym):
                        continue
                    if not isinstance(v, dict):
                        continue

                    name = str(
                        v.get('name') or base.get(
                            sym, {}).get('name') or sym).strip()
                    atype = str(
                        v.get('type') or base.get(
                            sym, {}).get('type') or 'stock').strip().lower()
    ...
    def update_prices():
        ...
        for asset in assets:
            ...
            # Approximate price_change_24h (not accurate but functional)
            # Better: if we have history, use it. If not, just random
            # fluctuation.

            # Let's keep existing price_change_24h logic if possible, or simulate it
            # Simulating 24h change: decay old change towards 0 and add new
            # change
            current_change = asset.price_change_24h or 0
            # Decay factor 0.95, add new change * 10
            asset.price_change_24h = (
                current_change * 0.98) + (change_percent * 100)
