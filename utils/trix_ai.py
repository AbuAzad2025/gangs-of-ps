...
class TrixAILogic:
    ...
    def get_doubling_action(state, player_index):
        ...
        if contract in ['king', 'complex']:
            has_king = any(
                c['suit'] == '♥' and c['rank'] == 'K' for c in hand)
    ...
    def get_best_move(state, player_index):
        ...
        if contract == 'trix':
            return TrixAILogic._play_trix_contract(
                state, player_index, valid_moves)
        else:
            return TrixAILogic._play_trick_contract(
                state, player_index, valid_moves, contract,
                is_partnership, partner_index)
    ...
    def get_valid_moves(state, player_index):
        ...
        if contract == 'trix':
            ...
            # (if game logic requires explicit pass, handled by server)
            # In this AI, we return empty list if pass is needed,
            # or handle pass as a special move
    ...
    def _play_trix_contract(state, player_index, valid_moves):
        ...
        - Avoid opening up piles that help opponents
          (unless I have cards there).
        - Heuristic: Count how many cards I have in each suit.
        """
        if not valid_moves:
            # Special pass card
            return {'suit': '♥', 'rank': '2', 'action': 'pass'}

        # 1. Prioritize playing Jacks to open piles
        # if I have many cards in that suit
        ...
        if jacks:
            ...
            best_jack = max(
                jacks,
                key=lambda c: sum(1 for h in hand if h['suit'] == c['suit']))
            return best_jack
    ...
    def _play_trick_contract(state, player_index, valid_moves, contract, is_partnership, partner_index):
        ...
        current_winner_is_partner = (
            (current_winner_idx == partner_index) if is_partnership else False
        )
        # Analyze the trick for danger
        trick_cards = [t['card'] for t in trick]
        danger_points = TrixAILogic._calculate_danger(
            trick_cards, contract, state.get('doubles', {}))
        ...
        if not lead_suit_moves:
            ...
            if current_winner_is_partner:
                if danger_points > 0:
                    ...
                    # card that is LOWER than theirs? No that doesn't matter.
                    # Just play highest possible card that is LOWER than
                    # partner's card to "save" my low cards for later?
                    # Or play highest card generally to get rid of it?
                    # If I play higher than partner, *I* take the points.
                    # "Partners don't hurt each other":
                    # If I take the points, it's same for team.
                    # But maybe I have x2 on me?
                    # Simple logic: If partner winning negative trick,
                    # let them win (play low).
                    # UNLESS I can win and I have a better multiplier
                    # (unlikely to know here).
                    # Play lowest card.
                    return lead_suit_moves[0]
                else:
                    # Clean trick. Partner winning.
                    # Great! Play highest card possible (that is still lower
                    # than partner's, or higher? No if higher I take it).
                    # If I play higher, I steal the clean trick.
                    # Does it matter? Yes, maybe I want the lead.
                    # But generally, save high cards.
                    # Play high card that is LOWER than partner.
                    # If I can't stay lower, I have to take it.
                    # winner_card = trick[0]['card'] # Simplified
                    # Find cards lower than current max
                    lower_moves = [
                        c for c in lead_suit_moves
                        if get_card_value(c) < highest_val_on_table]
                    if lower_moves:
                        return lower_moves[-1]  # Highest of the lowers
                    else:
                        return lead_suit_moves[0]  # Lowest of the highers
            # Scenario B: Opponent is winning.
                ...
                if danger_points > 0:
                    ...
                    if lower_moves:
                        ...
                        # Play the HIGHEST card to get rid of it?
                        # Or LOWEST to save high?
                        # If I'm eating it anyway, play the highest to
                        # drain my hand of power.
                        return lead_suit_moves[-1]
                else:
                    # Clean trick (Slapping/Latrosh always has -15,
                    # so never clean technically unless specific cards)
                    ...
                    # If King/Queens/Diamonds and no special cards,
                    # it's safe (0 points).
