...
class MyAdminIndexView(AdminIndexView):
    ...
    def index(self):
        ...
        recent_logs = GameLog.query.order_by(GameLog.timestamp.desc()).limit(5).all()
        pending_transactions = PaymentTransaction.query.filter_by(status='pending').order_by(PaymentTransaction.created_at.desc()).limit(5).all()
        
        return self.render('admin/index.html', stats=stats, recent_users=recent_users, recent_logs=recent_logs, pending_transactions=pending_transactions)

admin = Admin(name='Gangs of Palestine', index_view=MyAdminIndexView(), theme=Bootstrap4Theme())
babel = Babel()
csrf = CSRFProtect()
