    ...
    def decorator(f):
        ...
        def decorated_function(*args, **kwargs):
            ...
            if current_user.role.value < role.value:
                flash(
                    _('ليس لديك صلاحية للوصول إلى هذه الصفحة.'), 'danger')
    ...
    def decorated_function(*args, **kwargs):
        ...
        try:
            ...
            if datetime.now(timezone.utc) - verified_at > timedelta(
                ...
                session['next_url'] = request.url
                flash(
                    _('انتهت جلسة التحقق، يرجى التأكيد مرة أخرى.'), 'warning')
                return redirect(url_for('main.admin_verify'))
        except Exception:
            session.pop('admin_verified_at', None)
            session['next_url'] = request.url
        ...
        def decorated_function(*args, **kwargs):
            ...
            if feature_key:
                if SystemConfig.get_value(
                        f'maintenance_{feature_key}') == 'true':
                    if not current_user.is_authenticated or \
                            current_user.role.value < UserRole.ADMIN.value:
                        flash(
                            _('هذه الميزة معطلة مؤقتاً للصيانة.'), 'warning')
                        return redirect(url_for('main.index'))
    ...
    def decorated_function(*args, **kwargs):
        ...
        if current_user.role.value >= UserRole.MODERATOR.value:
            ...
            if current_user.role != UserRole.DEVELOPER:
                flash(
                    _(
                        'لا يُسمح للإداريين بالمشاركة في اللعب لضمان النزاهة.'
                    ), 'danger')
                return redirect(url_for('main.index'))
