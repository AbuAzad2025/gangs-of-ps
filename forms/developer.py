...
class ItemForm(FlaskForm):
    ...
    ammo_needed = IntegerField('Ammo Needed (per attack)', validators=[Optional()])
    ...
    image = FileField('Item Image', validators=[FileAllowed(['jpg', 'png', 'jpeg'], 'Images only!')])
...
class CrimeForm(FlaskForm):
    ...
    money_reward_min = IntegerField('Min Money Reward', validators=[DataRequired()])
    money_reward_max = IntegerField('Max Money Reward', validators=[DataRequired()])
    ...
    reward_item_id = SelectField('Reward Item (If Item Type)', coerce=int, validators=[Optional()])
    image = FileField('Crime Image', validators=[FileAllowed(['jpg', 'png', 'jpeg'], 'Images only!')])
...
class TaskForm(FlaskForm):
    ...
    image = FileField('Task Image', validators=[FileAllowed(['jpg', 'png', 'jpeg'], 'Images only!')])
    is_active = BooleanField('Active')
...
class OrganizedCrimeForm(FlaskForm):
    name = StringField('Crime Name', validators=[DataRequired()])
    description = TextAreaField('Description')
    min_members = IntegerField('Min Members', default=3, validators=[DataRequired()])
    min_gang_level = IntegerField('Min Gang Level', default=1, validators=[DataRequired()])
    energy_cost = IntegerField('Energy Cost (Leader)', default=50, validators=[DataRequired()])
    min_level = IntegerField('Min Level', default=10, validators=[DataRequired()])
    cooldown_hours = IntegerField('Cooldown Hours', default=24, validators=[DataRequired()])
    money_reward_min = IntegerField('Min Money Reward', default=1000, validators=[DataRequired()])
    money_reward_max = IntegerField('Max Money Reward', default=5000, validators=[DataRequired()])
    exp_reward = IntegerField('EXP Reward', default=100, validators=[DataRequired()])
    image = FileField('Crime Image', validators=[FileAllowed(['jpg', 'png', 'jpeg'], 'Images only!')])
    is_active = BooleanField('Active')
...
class HostessForm(FlaskForm):
    ...
    knowledge_base = TextAreaField(_l('قاعدة المعرفة (دليل المستخدم)'), render_kw={"rows": 10})
    training_examples = TextAreaField(_l('أمثلة تدريب (JSON)'), render_kw={"rows": 10, "placeholder": '[{"role": "user", "content": "..."}, {"role": "assistant", "content": "..."}]'})
    
    # Video Configuration
    video_choice = SelectField(_l('اختيار أفاتار موجود'), choices=[('', _l('بدون'))], validators=[Optional()])
    video = FileField(_l('أفاتار متحرك (MP4/WebM/GIF)'), validators=[FileAllowed(['mp4', 'webm', 'gif'], 'Videos only!')])
    video_prompt = TextAreaField(_l('سيناريو الفيديو (JSON)'), render_kw={"rows": 10, "placeholder": '{"title": "...", "description": "...", "shots": [...]}'})
    
    # Voice & Personality
    voice_config = TextAreaField(_l('إعدادات الصوت (JSON)'), render_kw={"rows": 5, "placeholder": '{"provider": "browser", "pitch": 1.0, "rate": 1.0}'})
    personality_config = TextAreaField(_l('إعدادات الشخصية (JSON)'), render_kw={"rows": 5, "placeholder": '{"flirt_level": 5, "shyness": 2}'})
    ...
    clothing_custom = StringField(_l('وصف الملابس (تخصيص)'), render_kw={"placeholder": "مثال: فستان أحمر قصير ضيق..."})
    hair_style = StringField(_l('ستايل الشعر'), render_kw={"placeholder": "مثال: شعر أسود طويل مموج..."})
    body_features = StringField(_l('مواصفات الجسم'), render_kw={"placeholder": "مثال: قوام رياضي، عيون عسلية..."})
    
    appearance_config = TextAreaField(_l('تكوين المظهر الكامل (JSON)'), render_kw={"rows": 5, "readonly": True})
    ...
    image = FileField(_l('صورة المضيفة'), validators=[FileAllowed(['jpg', 'png', 'jpeg'], 'Images only!')])
...
class LocationForm(FlaskForm):
    ...
    image = FileField(_l('صورة الموقع'), validators=[FileAllowed(['jpg', 'png', 'jpeg'], 'Images only!')])
    submit = SubmitField(_l('حفظ الموقع'))

class HostessKnowledgeForm(FlaskForm):
    question = TextAreaField(_l('السؤال / الموضوع'), validators=[DataRequired()])
    answer = TextAreaField(_l('الإجابة / المعلومات'), validators=[DataRequired()])
    category = StringField(_l('التصنيف'), default='general')
    keywords = StringField(_l('كلمات مفتاحية (مفصولة بفاصلة)'))
    language = SelectField(_l('اللغة'), choices=[('ar', 'العربية'), ('en', 'English')], default='ar')
    hostess_id = SelectField(_l('مخصص لمضيفة (اختياري)'), coerce=int, validators=[Optional()])
...
class VideoScenarioForm(FlaskForm):
    ...
    description = TextAreaField(_l('الوصف'))
    script_content = TextAreaField(_l('محتوى السيناريو (JSON)'), validators=[DataRequired()], render_kw={"rows": 10})
...
class FactoryJobForm(FlaskForm):
    ...
    ends_at = DateTimeField(_l('وقت الانتهاء (YYYY-MM-DD HH:MM:SS)'), format='%Y-%m-%d %H:%M:%S', validators=[DataRequired()])
...
class MarketAssetForm(FlaskForm):
    ...
    base_price = IntegerField(_l('السعر الأساسي'), default=100, validators=[DataRequired()])
    volatility = IntegerField(_l('التقلب (0-100%)'), default=5, validators=[DataRequired()])
    description = TextAreaField(_l('الوصف'))
    is_active = BooleanField(_l('نشط'), default=True)
