...
IMAGES_TO_DOWNLOAD = {
    ...
    'crimes/smuggling.jpg': 'Intermodal_shipping_containers_on_a_railway_flat_car.jpg',
    'crimes/cyber.jpg': 'Binary_1-7.PNG',
    'crimes/bank_heist.jpg': 'Bank_of_Quitman_Interior_03.jpg',
    'crimes/armored_van.jpg': 'CTK_armored_van_on_the_Philippine_island_of_Palawan.jpg',
    ...
    'items/golden_ak47.jpg': 'AK-47_assault_rifle.jpg', # Using standard for now, gold is rare on commons
    'items/m16.jpg': 'M16A1_rifle.jpg',
    'items/light_vest.jpg': 'Giubetto_Antiproiettile_-_standard_issue_balllstic_vest.png', 
    ...
    'items/explosives.jpg': '160mm_Mortar_bomb_HE_TNT,_photographed_at_the_Aalborg_Forsvars-_og_Garnisonsmuseum.JPG',
    'items/molotov.jpg': 'Detail_of_Molotov_Cocktail_-_Museum_of_the_Great_Patriotic_War_-_Minsk_-_Belarus_(26916185674).jpg',
    ...
    'vehicles/mercedes_g_class.jpg': 'Mercedes_G-Klasse_Edition_Pur_20090808_front.JPG',
}
...
def get_image_url(filename):
    ...
    try:
        ...
        match = re.search(r'href="(https://upload\.wikimedia\.org/wikipedia/commons/[^"]+)" class="internal"', r.text)
        if match: return match.group(1)
        
        # Try 2: Full Media div
        match = re.search(r'<div class="fullMedia"><a href="(https://upload\.wikimedia\.org/wikipedia/commons/[^"]+)"', r.text)
        if match: return match.group(1)
        
        # Try 3: contentUrl property
        match = re.search(r'"contentUrl":\s*"(https://upload\.wikimedia\.org/wikipedia/commons/[^"]+)"', r.text)
        if match: return match.group(1)
...
def download_file(url, local_path):
    try:
        ...
        if r.status_code == 200:
            ...
            print(f"[-] Failed to download content from {url}: {r.status_code}")
            return False
    except Exception as e:
...
def main():
    ...
    for local_rel, wiki_name in IMAGES_TO_DOWNLOAD.items():
        ...
        if image_url:
            ...
            # Try to create a dummy placeholder if it fails? No, better to leave it missing so we know.
