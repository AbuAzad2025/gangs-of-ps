...
def main():
    ...
    with app.app_context():
        ...
        user2 = _find_user(
            args.user2) if args.user2 else _pick_second_user(
            azad.id)
        if not user2:
            raise SystemExit("Second user not found")

        azad_until = (now + timedelta(minutes=max(1,
                                                  int(args.minutes_azad)))).replace(tzinfo=None)
        user2_until = (now + timedelta(minutes=max(1,
                                                   int(args.minutes_user2)))).replace(tzinfo=None)

        ResourceService.modify_resources(
            azad.id, {}, "admin_jail", auto_commit=True, set_fields={
                "jail_until": azad_until})
        ResourceService.modify_resources(
            user2.id, {}, "admin_jail", auto_commit=True, set_fields={
                "jail_until": user2_until})
