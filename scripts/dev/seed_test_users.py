...
def _ensure_user(username: str, password: str, role: UserRole, level: int) -> User:
...
def main():
    ...
    with app.app_context():
        now = _now()
        roles = [UserRole.USER, UserRole.USER, UserRole.USER, UserRole.MODERATOR, UserRole.ADMIN, UserRole.DEVELOPER]
        ...
        for i in range(jail_count):
            users[i].jail_until = (now + timedelta(minutes=45 + (i * 15))).replace(tzinfo=None)
