...
def fix_locations():
    with app.app_context():
        ...
        for loc in locations:
            # Check if image is just a filename (no slash) and exists in
            # static/images/locations
            if loc.image and '/' not in loc.image:
                possible_path = os.path.join(
                    app.root_path, 'static', 'images', 'locations', loc.image)
                if os.path.exists(possible_path):
                    print(
                        f"Fixing location {loc.name}: {loc.image} -> locations/{loc.image}")
                    loc.image = f"locations/{loc.image}"
                    count += 1
                elif loc.image == 'default_city.jpg':
                    # Special case for default, fix it anyway even if file
                    # missing
                    print(
                        f"Fixing location {loc.name}: {loc.image} -> locations/{loc.image}")
                    loc.image = f"locations/{loc.image}"
