            ...
            with db.engine.connect() as conn:
                ...
                try:
                    ...
                    if 'hostesses' not in inspector.get_table_names():
                        print(
                            "Table 'hostesses' does not exist. Creating all tables...")
                        db.create_all()
                    ...
                    if 'level' not in columns:
                        conn.execute(
                            text("ALTER TABLE hostesses ADD COLUMN level INTEGER DEFAULT 1"))
                        print("Added level")

                    if 'exp' not in columns:
                        conn.execute(
                            text("ALTER TABLE hostesses ADD COLUMN exp INTEGER DEFAULT 0"))
                        print("Added exp")

                    if 'charm' not in columns:
                        conn.execute(
                            text("ALTER TABLE hostesses ADD COLUMN charm INTEGER DEFAULT 10"))
                        print("Added charm")

                    if 'intelligence' not in columns:
                        conn.execute(
                            text("ALTER TABLE hostesses ADD COLUMN intelligence INTEGER DEFAULT 10"))
                        print("Added intelligence")

                    if 'combat_skill' not in columns:
                        conn.execute(
                            text("ALTER TABLE hostesses ADD COLUMN combat_skill INTEGER DEFAULT 0"))
                        print("Added combat_skill")

                    if 'loyalty' not in columns:
                        conn.execute(
                            text("ALTER TABLE hostesses ADD COLUMN loyalty INTEGER DEFAULT 50"))
                        print("Added loyalty")

                    if 'special_move_cooldown' not in columns:
                        conn.execute(
                            text("ALTER TABLE hostesses ADD COLUMN special_move_cooldown TIMESTAMP"))
