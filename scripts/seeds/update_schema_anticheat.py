...
def update_schema():
    with app.app_context():
        ...
        with db.engine.connect() as conn:
            ...
            result = conn.execute(
                text("SELECT column_name FROM information_schema.columns WHERE table_name='user' AND column_name='is_suspicious'"))
            if not result.fetchone():
                print("Adding is_suspicious column to user table...")
                conn.execute(
                    text("ALTER TABLE \"user\" ADD COLUMN is_suspicious BOOLEAN DEFAULT FALSE"))
                conn.commit()
                print("Column added.")
            else:
