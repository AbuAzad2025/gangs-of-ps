...
def add_columns():
    with app.app_context():
        ...
        try:
            ...
            # Fallback for some drivers if 'user' table case sensitivity is an
            # issue
            columns = [c['name'] for c in inspector.get_columns('User')]
        ...
        with engine.connect() as conn:
            ...
            try:
                ...
                if 'last_energy_update' not in columns:
                    ...
                    conn.execute(
                        text('ALTER TABLE "user" ADD COLUMN last_energy_update TIMESTAMP'))
                ...
                if 'last_health_update' not in columns:
                    ...
                    conn.execute(
                        text('ALTER TABLE "user" ADD COLUMN last_health_update TIMESTAMP'))
